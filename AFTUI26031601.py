...
# --- [DB 설정: HS 마스터는 로컬(AGS 폴더) / 유저는 구글시트] ---
# 바탕화면 AGS 폴더 경로 설정 (사용자명 자동 인식으로 개선)
def get_db_path():
    desktop = os.path.join(os.path.expanduser("~"), "Desktop")
    ags_dir = os.path.join(desktop, "AGS")
    if not os.path.exists(ags_dir):
        try: os.makedirs(ags_dir)
        except: return "customs_master.db" # 권한 문제 시 현재 폴더에 생성
...
try:
    ...
    if not df_users.empty:
        ...
        st.dataframe(df_users.head(2)) # 상위 2개만 출력
    else:
        st.warning("⚠️ 시트 연결은 되었으나 데이터가 비어있습니다. 헤더와 내용을 확인하세요.")
