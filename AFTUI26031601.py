...
# --- [DB 설정: HS 마스터 정보는 로컬 유지 / 유저는 구글시트로 전환] ---
# HS 정보 및 통계부호는 기존처럼 바탕화면 AGS 폴더의 DB에서 관리 (조회 속도 최적화)
db_path = r"C:\Users\USER\Desktop\AGS\customs_master.db"

if not os.path.exists(r"C:\Users\USER\Desktop\AGS"):
    try: os.makedirs(r"C:\Users\USER\Desktop\AGS")
    except: pass
...
# --- 2. 로그인 세션 (구글 시트 데이터 기반으로 수정) --- ★구글시트방식 반영
    ...
    with cl2:
        with st.form("login_form"):
            ...
            if st.form_submit_button("로그인"):
                # 구글 시트에서 가져온 df_users에서 일치하는 계정 검색
        ...
        if not st.session_state.admin_verified:
            ...
            with adm_tabs[1]:
                ...
                with st.expander("➕ 신규 화주 등록 및 구글 시트 저장"):
                    ...
                    nid = c1.text_input("아이디(ID)")
                    nnm = c2.text_input("성명/업체명")
                    ...
                    if st.button("구글 시트에 계정 추가"):
                        if nid and nnm:
                            ...
                            # 신규 행 데이터 준비
                            new_row = pd.DataFrame([{"id": nid, "pw": hashed_pw, "name": nnm, "is_approved": 1, "is_admin": 0, "access_tabs": "ALL"}])
                            # 구글 시트에 업데이트 (기존 데이터 + 신규 데이터)
                            updated_df = pd.concat([df_users, new_row], ignore_index=True)
                            conn_gs.update(worksheet="users", data=updated_df)
                            st.success(f"✅ {nnm} 계정이 구글 드라이브 시트에 저장되었습니다.")
                ...
                if not df_users.empty:
                    st.dataframe(df_users[["id", "name", "is_approved"]], use_container_width=True, hide_index=True)
                    
                    target = st.selectbox("관리 대상 선택", df_users[df_users["is_admin"]==0]["id"].tolist())
                    
                    if st.button("🔑 선택 계정 비밀번호 1234로 초기화"):
                        ...
                        st.info(f"💡 {target}의 비밀번호가 초기화되어 구글 시트에 반영되었습니다.")
