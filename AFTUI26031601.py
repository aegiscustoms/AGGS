        ...
        if items:
            ...
            for item in items:
                post_data_list.append({
                    "진행단계": item.findtext("psmtPrcsTpcdNm") or "-",
                    "처리상태": item.findtext("psmtPrcsStcd") or "-",
                    "반입일": fmt_d(item.findtext("brngArvlDt")),
                    "통관일": fmt_d(item.findtext("aprvDt")),
                    "장소": item.findtext("csclPsofNm") or "-",
                    "관리번호": item.findtext("psmtCsclMtNo") or "-"
                })
            
            if post_data_list:
                df_post = pd.DataFrame(post_data_list)
                st.write("**📑 우편물 상세 진행 내역**")
