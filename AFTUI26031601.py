...
- 통칙 제1호(호의 용어 및 주 규정)를 최우선 적용하고, 제6호까지 논리적으로 순차 적용할 것.
- 만약 예상되는 4단위 호가 경합될 경우, 본질적인 특성을 부여하는 재질/기능에 따라 확정할 것.

[리포트 출력 양식]
1. 입력정보
   1) 이미지 해석: (이미지가 있다면 시각적 특징과 식별 포인트 서술, 없으면 '정보 없음')
{input_summary}
...
3. 분류 근거 (다음 3개 문단 구조를 반드시 지킬 것)
   - 1문단: 물품이 속하는 부/류의 분류 규정(주 규정) 및 해설서 총설의 핵심 내용 서술
   - 2문단: 확정된 4단위 호의 용어 설명 및 호해설서에서 해당 물품을 분류할 수 있는 구체적 근거 예시 인용
   - 3문단: 물품의 특징(재질/기능 등)을 요약하고 통칙 제1호 및 제6호를 적용하여 최종 번호가 산출되는 논리 결론

4. 예상 경합세번 (추천 번호의 확률이 100%가 아닌 경우만 표시)
