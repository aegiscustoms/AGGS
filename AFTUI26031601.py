...
# --- [Tab 5] 세액계산기 (전체 국가 리스트 내장 및 API 연동) --- #변경내역★★
with tabs[4]:
    st.markdown("<div class='custom-header'>💰 수입물품 예상 세액계산기 및 환율정보</div>", unsafe_allow_html=True)
    ...
    # [1] 관세청 고시환율 대상 58개국 리스트 (엑셀 데이터 기반)
    currency_options = [
    ...
    # [2] 환율 조회 섹션
    with st.container(border=True):
        st.write("**🔎 관세청 주간 고시환율 조회**")
        
        ex_c1, ex_c2, ex_c3, ex_c4 = st.columns([1, 1.5, 1, 1])
        
        with ex_c1:
            exch_tp = st.selectbox("환율구분", ["수입 (2)", "수출 (1)"], index=0, key="v5_ex_tp_final")
            imex_tp = exch_tp.split("(")[1][0]
            
        with ex_c2:
            # 자동완성 기능이 포함된 selectbox
            selected_curr_full = st.selectbox("국가 또는 통화코드 선택/입력", currency_options, index=0, key="v5_curr_auto")
            target_curr = selected_curr_full.split(" ")[0]

        with ex_c3:
            st.write(" ") 
            if st.button("관세청 환율 조회", use_container_width=True, type="primary", key="v5_ex_api_btn"):
                API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                if not API_KEY:
                    st.error("❌ API 키 설정 누락")
                else:
                    with st.spinner(f"{target_curr} 환율 조회 중..."):
                            ...
                            if res.status_code == 200:
                                ...
                                for item in items:
                                    if item.findtext("currSgn") == target_curr:
                                        rate_str = item.findtext("fxrt")
                                        rate_val = float(rate_str.replace(",", ""))
                                        st.session_state.api_ex_rate = rate_val
                                        st.session_state.api_ex_disp = f"{target_curr}: {rate_val:,.2f}원"
                                        found = True
                                        break
                                if not found:
                                    st.warning(f"⚠️ {target_curr} 정보 없음")
                            else: st.error(f"❌ API 오류: {res.status_code}")
                        except Exception as e: st.error(f"❌ 통신 오류: {e}")

        with ex_c4:
            st.write(" ") 
            if st.session_state.api_ex_disp:
                st.markdown(f"""
                    <div style='background-color: #F0F9FF; padding: 8px; border: 1px solid #0369A1; 
                    border-radius: 5px; text-align: center; font-weight: bold; color: #0369A1; font-size: 14px;'>
                        {st.session_state.api_ex_disp}
                    </div>
                """, unsafe_allow_html=True)

    st.divider()

    # [3] 과세가격(CIF) 및 품목 입력 영역
    with st.container(border=True):
        ...
        with cr:
            # 조회된 환율 자동 반영
            p_ex = st.number_input("적용 환율 (원)", value=st.session_state.api_ex_rate, format="%.2f", key="v5_ex_rate_p")
            
            st.write("품목분류(HSK)")
            h1, h2 = st.columns([0.7, 0.3])
            hs_in = h1.text_input("HSK 입력", label_visibility="collapsed", key="v5_hs_p", placeholder="예: 0101211000")
            
            if h2.button("적용", key="v5_hs_btn"):
                if hs_in:
                    ...
                    if not r_df.empty:
                        ...
                        st.success(f"HSK {hsk_clean} 적용 완료")
                        st.rerun()

            r1, r2 = st.columns(2)
            a_d = r1.number_input(f"관세율({st.session_state.calc_t}, %)", value=st.session_state.calc_d, key="v5_duty_p")
            a_v = r2.number_input("부가세율(%)", value=10.0, key="v5_vat_p")
        
        cif = int((p_price * p_ex) + p_frt + p_ins)
        st.info(f"**과세표준 (CIF KRW): {cif:,.0f} 원**")
        
    # [4] 세액 계산 실행 및 결과 출력
    if st.button("세액 계산 실행", use_container_width=True, type="primary", key="v5_main_btn"):
        d = int(cif * (a_d/100))
        v = int((cif + d) * (a_v/100))
        
        st.markdown(f"<div style='font-size: 22px; font-weight: bold; color: #B91C1C; text-align: right; background-color: #FEF2F2; padding: 15px; border-radius: 8px;'>💰 예상세액 합계: {d+v:,.0f} 원</div>", unsafe_allow_html=True)
        
        st.write("**📑 세액 산출 내역**")
        t_style = 'style="width:100%; border-collapse:collapse; border:1px solid #E2E8F0;"'
        th_style = 'style="background-color:#F8FAFC; color:#1E3A8A; text-align:center; padding:12px; border:1px solid #E2E8F0; font-size:14px;"'
    ...
    st.markdown("""
        <div style="font-size: 14px; color: #64748B; margin-top: 25px; line-height: 1.6; border-top: 1px solid #E2E8F0; padding-top: 15px;">
            ※ 위 환율은 관세청 현재 주간 고시환율 기준입니다.<br>
            ※ 개별소비세, 주세, 교육세 등 내국세 부과대상의 예상세액은 관세사와 상담 부탁드립니다.<br>
            ※ 예상세액은 참조용이며, 정확한 세액 산출은 관세사 상담을 권장합니다.
        </div>
    """, unsafe_allow_html=True)
