...
# --- [Tab 2] HS정보 (모바일 최적화 및 3cm 고정폭 반영) --- #변경내역★★
with tabs[1]:
                ...
                if not m.empty or not std.empty:
                    ...
                    if not r_all.empty:
                        ...
                        def styled_rate_table(df, is_scroll=False):
                            # 테이블 스타일: 화면 꽉 채움 + 자동 줄바꿈(word-break) 적용 #변경내역★★
                            table_style = 'style="width:100%; border-collapse:collapse; table-layout:fixed; border:1px solid #E2E8F0; word-break:break-all;"'
                            th_style = 'style="background-color:#F8FAFC; color:#1E3A8A; text-align:center; padding:8px 4px; border:1px solid #E2E8F0; font-size:14px; position:sticky; top:0; z-index:10;"'
                            ...
                            # [보정] 코드와 세율 열을 3cm(약 113px)로 설정 #변경내역★★
                            html += '<colgroup><col style="width:113px;"><col style="width:auto;"><col style="width:113px;"></colgroup>'
                            ...
                            if is_scroll:
                                return f'<div style="max-height:450px; overflow-y:auto; border-radius:4px; border:1px solid #E2E8F0; width:100%;">{html}</div>'
                            return f'<div style="width:100%; overflow-x:hidden;">{html}</div>'
