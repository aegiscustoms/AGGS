...
with tabs[3]:
    ...
    # [복구] 입력창: 세션 변수와 직접 연결 (B/L번호 혹은 화물관리번호만으로도 조회 가능) #변경내역★★
    with c_b: 
        bl_input = st.text_input("B/L 번호", value=st.session_state.bl_val, key="bl_search_v4")
    with c_m: 
        mrn_input = st.text_input("화물관리번호", value=st.session_state.mrn_val, key="mrn_search_v4")
    ...
    if st.button("실시간 조회 실행", use_container_width=True):
        ...
        if not API_KEY:
            ...
            with st.spinner("관세청 유니패스 통신 중..."):
                ...
                # [보정] B/L 혹은 화물관리번호 중 하나라도 있으면 조회 시도 #변경내역★★
                params = {
                    "crkyCn": API_KEY, 
                    "blYy": str(carg_year), 
                    "hblNo": bl_input.upper().strip() if bl_input else "", 
                    "cargMtNo": mrn_input.upper().strip() if mrn_input else ""
                }
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if info is not None:
                            ...
                            st.warning("⚠️ 조회 결과가 없습니다. 번호와 입항년도를 확인해 주세요.")
                except Exception as e:
                    st.error(f"❌ 통신 오류: {e}")
