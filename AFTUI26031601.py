...
# --- [Tab 1] HS검색 (UI 개편 및 검토의뢰 시스템) --- #변경내역★★
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        with c1:
            uploaded_file = st.file_uploader("📸 물품 이미지 업로드", type=["jpg", "jpeg", "png"], key="hs_img_v1")
            if uploaded_file:
    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not any([uploaded_file, p_name, p_material, p_usage, p_function, p_component, p_spec, p_composition]):
            st.warning("⚠️ 분석을 위해 이미지 업로드 또는 최소 하나 이상의 물품 정보를 입력해주세요.")
        else:
            with st.spinner("제미나이가 전문 자료를 분석 중입니다..."):
                try:
                    model = genai.GenerativeModel('gemini-1.5-flash')
                    content = ["당신은 전문 관세사입니다. 다음 입력된 정보를 바탕으로 예상되는 HS Code 10자리를 추천하고 분류 근거를 상세히 설명하는 'HS 품목분류 분석 리포트'를 작성하세요.", input_data]
                    if uploaded_file:
                    ...
                    st.session_state.last_report = report_text # 메일 전송용 세션 저장
                    
                    st.markdown("### 📋 AI 분석 결과")
                    st.markdown(report_text)
                    
                    # [3] 관세사 검토의뢰 시스템 (리포트 생성 후에만 노출) #변경내역★★
                    st.divider()
                    ...
                    if col_req2.button("관세사 검토의뢰", use_container_width=True):
                        if not user_org:
                            ...
                            SMTP_PORT = 587
                            SENDER_EMAIL = "jhlee@aegiscustoms.com" # 발신 메일
                            # 주의: 아래 APP_PASSWORD는 구글에서 발급받은 16자리 앱 비밀번호를 입력해야 합니다.
                            # st.secrets["EMAIL_PASSWORD"] 형식을 권장합니다.
                            SENDER_PASSWORD = st.secrets.get("EMAIL_PASSWORD", "실제앱비밀번호입력") 

                            msg = MIMEMultipart()
                            msg['From'] = SENDER_EMAIL
                            msg['To'] = "jhlee@aegiscustoms.com" # 수신 메일 (관세사님 계정)
                            ...
                            * 본 메일은 AEGIS 포털 시스템을 통해 자동 발송되었습니다.
                            """
                            ...
                            try:
                                ...
                                server.quit()
                                st.success(f"✅ {user_org}님, 검토의뢰가 성공적으로 접수되었습니다. 곧 연락드리겠습니다.")
                            except Exception as e:
                                st.error(f"❌ 메일 전송 중 오류가 발생했습니다: {e}")
                except Exception as e:
