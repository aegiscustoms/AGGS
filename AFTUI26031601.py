...
# --- [1. 구글 시트 연결 및 데이터 로드] --- ★ 최우선 실행 (순서 변경)
try:
    # 스트림릿 Secrets 정보를 바탕으로 구글 시트에 연결합니다.
    conn_gs = st.connection("gsheets", type=GSheetsConnection)
    # 구글 드라이브 시트의 'users' 워크시트에서 계정 정보를 읽어옵니다.
    df_users = conn_gs.read(worksheet="users", ttl=0)
except Exception as e:
    # 연결 전이거나 오류 시 빈 데이터프레임 생성 및 에러 출력
    st.error(f"❌ 구글 시트 초기 연결 실패: {e}")
    ...
    with cl2:
        with st.form("login_form"):
            ...
            if st.form_submit_button("로그인"):
                ...
                user_match = df_users[(df_users['id'] == l_id) & (df_users['pw'] == input_pw_hash)]
                if not user_match.empty:
                    user_info = user_match.iloc[0]
                    if int(user_info['is_approved']) == 1:
                        st.session_state.logged_in = True
                        st.session_state.user_id = l_id
                        st.session_state.user_name = user_info['name']
                        st.session_state.is_admin = bool(int(user_info['is_admin']))
                        st.rerun()
