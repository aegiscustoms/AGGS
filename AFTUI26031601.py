...
# --- [Tab 1] HS검색 (이미지 파일 실제 메일 첨부 로직 추가) --- #변경내역★★
with tabs[0]:
    ...
    if "last_img_bytes" not in st.session_state: st.session_state.last_img_bytes = None # 이미지 저장을 위한 세션
    ...
    with st.container(border=True):
        ...
        with c1:
            ...
            if uploaded_file:
                ...
                # 메일 첨부를 위해 이미지 바이트를 세션에 저장 #변경내역★★
    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("AI 전문 관세사가 분석 중..."):
                try:
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    prompt = f"당신은 30년 경력의 베테랑 관세사입니다. 다음 양식에 맞춰 분석 리포트를 작성하세요.\n\n[리포트 양식]\n1. 입력정보\n   1) 이미지 해석: (시각 특징 서술)\n{input_summary}\n\n2. 추천 HS Code (10자리)\n3. 분류 근거\n4. 예상 경합세번"
    ...
    if st.session_state.ai_report_done:
        ...
        if col_req2.button("관세사 검토의뢰", use_container_width=True):
            if not user_org:
                ...
                from email.mime.application import MIMEApplication # 이미지 첨부용 #변경내역★★
...
"""
                    ...
                    if st.session_state.last_img_bytes:
                        try:
                            ...
                            st.warning(f"이미지 첨부 중 오류 발생: {img_err}")
                    ...
                    try:
                        ...
                        server.quit()
                        st.success(f"✅ {user_org}님, 검토의뢰 메일(이미지 첨부됨)이 관세사에게 전달되었습니다.")
                    except Exception as e:
