...
# --- [추가] DB 경로 및 폴더 설정 (관세사님 지정 경로: 바탕화면 AGS) ---
# 윈도우 사용자명이 'USER'가 아닐 경우 실제 이름으로 수정이 필요할 수 있습니다.
...
if not os.path.exists(db_dir):
    try:
        ...
        # 경로 접근 권한 문제 발생 시 현재 실행 폴더에 생성 (안전장치)
        db_path = "customs_master.db"

# --- [수정] 통합 DB 초기화 함수 (모든 테이블 및 관리자 계정 생성) ---
def init_db():
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    
    # 1. HS 마스터 및 요건/세율 관련 테이블 생성
    ...
    # 3. 사용자 및 권한 관리 테이블 생성 (비밀번호 상세제어용)
    ...
    # 관리자 계정이 없을 경우 기본 생성 (ID: admin, PW: admin1234)
    c.execute("SELECT * FROM users WHERE id='admin'")
    if not c.fetchone():
        # 비밀번호는 보안을 위해 SHA-256으로 해싱하여 저장합니다.
        admin_pw_hash = hashlib.sha256("admin1234".encode()).hexdigest()
        c.execute("INSERT INTO users (id, pw, is_approved, is_admin, name) VALUES (?, ?, 1, 1, ?)", 
                  ('admin', admin_pw_hash, '관리자'))
...
st.markdown(f"""
    <style>
        ...
        * {{ font-family: 'Pretendard', sans-serif; }}
        ...
        .custom-header {{ background-color: #F8FAFC; padding: 15px; border-radius: 8px; border-left: 5px solid #1E3A8A; margin-bottom: 20px; font-size: 18px; font-weight: bold; color: #1E3A8A; }}
    </style>
...
# --- [Main Logic: Tabs 정의] ---
tabs = st.tabs(["🏠 메인화면", "🔍 HS 정보", "📋 수입요건", "📦 통관정보", "💰 세액계산", "⚙️ 관리자"])
