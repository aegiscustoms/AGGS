    ...
    with cl2:
        with st.form("login_form"):
            ...
            if st.form_submit_button("로그인"):
                ...
                # 구글 시트 데이터 전처리: 모든 데이터를 문자열로 변환하여 비교 오류 방지
                df_users_str = df_users.astype(str)
                
                # 시트에서 아이디와 해시 비밀번호가 일치하는 행 탐색
                user_match = df_users_str[(df_users_str['id'] == l_id) & (df_users_str['pw'] == input_pw_hash)]
