...
# --- [Tab 2] HS정보 (테이블 레이아웃 강제 복구본) --- #변경내역★★
with tabs[1]:
                ...
                if not m.empty or not std.empty:
                    ...
                    if not r_all.empty:
                        ...
                        def styled_rate_table(df, is_scroll=False):
                            # 테이블 레이아웃을 강제로 지정하는 인라인 스타일 주입 #변경내역★★
                            table_style = 'style="width:100%; border-collapse:collapse; table-layout:fixed; border:1px solid #E2E8F0;"'
                            th_style = 'style="background-color:#F8FAFC; color:#1E3A8A; text-align:center; padding:12px; border:1px solid #E2E8F0; font-size:15px; position:sticky; top:0; z-index:10;"'
                            td_style = 'style="text-align:center; padding:10px; border:1px solid #F1F5F9; font-size:13px;"'
                            
                            # HTML 생성
                            html = f'<table {table_style}>'
                            # colgroup 주입 (코드 378px, 명칭 가변, 세율 378px)
                            html += '<colgroup><col style="width:378px;"><col style="width:auto;"><col style="width:378px;"></colgroup>'
                            ...
                            for _, row in df.iterrows():
                                html += '<tr>'
                                for val in row:
                                    html += f'<td {td_style}>{val}</td>'
                                html += '</tr>'
                            html += '</tbody></table>'
                            
                            if is_scroll:
                                return f'<div style="max-height:450px; overflow-y:auto; border-radius:4px; border:1px solid #E2E8F0;">{html}</div>'
                    ...
                    def styled_req_table(df):
                        # 요건 테이블용 스타일 (전체폭 + 중앙정렬) #변경내역★★
                        table_style = 'style="width:100%; border-collapse:collapse; border:1px solid #E2E8F0;"'
                        th_style = 'style="background-color:#F8FAFC; color:#1E3A8A; text-align:center; padding:12px; border:1px solid #E2E8F0;"'
                        td_style = 'style="text-align:center; padding:10px; border:1px solid #F1F5F9;"'
