            ...
            with st.spinner("🔍 관세청 통계 조회 및 AI 전문 분석 중..."):
                try:
                    ...
                    if NAVI_KEY and p_name:
                        ...
                        # 1차 시도: 품명(prlstNm)으로 직접 검색 (가장 정확한 방법)
                        params = {
                            "crkyCn": NAVI_KEY,
                            "prlstNm": p_name  # 품명을 직접 전달
                        } 
                        
                        try:
                            res = requests.get(url, params=params, timeout=10)
                            if res.status_code == 200:
                                # 유니패스 API 특성상 응답이 XML이므로 인코딩 설정 확인
                                res.encoding = 'utf-8'
                                root = ET.fromstring(res.text)
                                
                                # 가이드에 명시된 결과 리스트 태그 확인: cmtrStatsQryRsltVo
                                items = root.findall(".//cmtrStatsQryRsltVo")
                                
                                # 만약 품명으로 안나오면 2차 시도: HS코드 앞자리로 검색
                                if not items and len(p_name) >= 2: # 숫자가 포함된 경우 등 대비
                                    params_hs = {"crkyCn": NAVI_KEY, "hsSgn": p_name[:4]}
                                    res = requests.get(url, params=params_hs, timeout=10)
                                    root = ET.fromstring(res.text)
                                    items = root.findall(".//cmtrStatsQryRsltVo")
                            ...
                            stats_data = f"통계 조회 중 오류 발생: {str(api_err)}"
