...
# --- [Tab 5] 세액계산기 (안정성 강화 및 데이터 캐싱 적용) ---
with tabs[4]:
    st.markdown("""
        <style>
    ...
    st.markdown("<div class='custom-header'>💰 수입물품 예상 세액계산기 (AEGIS)</div>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        with c_left:
                                ...
                                for item in items:
                                    if item.findtext("currSgn") == t_curr_code:
                                        ...
                                        if r_str:
                                            ...
                                            break
                                if not found:
                                    st.session_state.api_ex_disp = "<span style='color:red;'>N/A</span>"
                            else:
                                st.session_state.api_ex_disp = "<span style='color:red;'>ERROR</span>"
                        except:
                            st.session_state.api_ex_disp = "<span style='color:red;'>RETRY</span>"
            ...
            with l_r3_1: p_frt = st.number_input("운임 (KRW)", min_value=0.0, key="v5_frt_final")
            with l_r3_2: p_ins = st.number_input("보험료 (KRW)", min_value=0.0, key="v5_ins_final")
        ...
        with c_right:
            ...
            st.markdown(f"<div class='cif-box-final'><span class='cif-text-final'>{c_val:,.0f} 원</span></div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
