...
with tabs[4]:
    ...
    with st.container(border=True):
        ...
        with c_left:
            ...
            with l_r1_2:
                # 사용자가 직접 수정도 가능하도록 value에 세션값 연결
            ...
            with l_r2[2]:
                if st.button("환율조회", use_container_width=True, key="v5_api_btn_v11"):
                    ...
                    if API_KEY:
                        ...
                        # 가이드 운영 URL 및 서비스명 결합
                        base_url = "https://unipass.customs.go.kr:38010/ext/rest/trifFxrtInfoQry/retrieveTrifFxrtInfo"
                        params = {"crkyCn": API_KEY, "qryYymmDd": q_date, "imexTp": im_tp_val}
                        
                        # 브라우저와 동일한 환경을 위한 헤더 설정
                        headers = {
            ...
            with l_r3_1: p_frt = st.number_input("운임 (Freight, KRW)", min_value=0.0, key="v5_frt_v11")
            with l_r3_2: p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0.0, key="v5_ins_v11")
