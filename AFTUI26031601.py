...
# --- [Tab 1] HS검색 (전문 리포트 양식 및 메일 분석 보정) --- #변경내역★★
with tabs[0]:
    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            st.warning("⚠️ 분석 정보를 입력해주세요.")
        else:
            with st.spinner("전문 관세사 페르소나로 분석 중..."):
                try:
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    
                    # [고도화 프롬프트] 관세사님의 실무 분류 방식을 학습시킴
                    prompt = f"""당신은 20년 경력의 베테랑 관세사입니다. 제공된 정보를 바탕으로 다음 '제미나이 리포트 양식'에 맞춰 분석 리포트를 작성하세요.

[분류 원칙]
1. 4단위 호의 용어(Heading)를 우선 확정할 것.
2. 관련 부/류의 주(Note) 규정과 해설서 총설을 반드시 검토할 것.
3. 통칙 제1호(호의 용어 및 주 규정)를 최우선으로 적용하고, 필요시 제6호까지 순차 적용할 것.

[리포트 양식]
1. 입력정보
   1) 이미지 해석: (이미지가 있다면 시각적 특징 서술, 없으면 '정보 없음')
{input_summary}

2. 추천 HS Code (10자리): XXXX.XX-XXXX (예상 확률 %)
   - 4단위 HS Code: XXXX [호의 용어 한글(영문)]
   - 6단위 HS Code: XXXX.XX [소호의 용어]
   - 10단위 HS Code: XXXX.XX-XXXX [품목명]

3. 분류 근거 (실무적 서술)
   - 1문단: 해당 물품이 속하는 부/류의 분류 규정 및 해설서 총설 내용 요약
   - 2문단: 4단위 및 하위 단위 호의 용어 상세 설명 및 해설서 예시 인용
   - 3문단: 물품 정보 요약 및 통칙 제1호/제6호 적용 과정을 통한 최종 번호 확정 논리

4. 예상 경합세번 (확률이 100%가 아닌 경우만 작성)
   - 6단위: XXXX.XX(00%), 10단위: XXXX.XX-XXXX(00%)

※ 주의사항 ※
- 본 리포트는 참고 자료이며 법적 책임을 지지 않습니다.
- 상세 상담 필요 시 하단 '관세사 검토의뢰'를 이용하십시오.
"""
                    content = [prompt]
                    if uploaded_file:
...
"""
                    ...
                    try:
                        ...
                        server.quit()
                        st.success(f"✅ {user_org}님, 검토의뢰 메일이 이미지 분석 내용을 포함하여 관세사에게 전달되었습니다.")
                    except Exception as e:
