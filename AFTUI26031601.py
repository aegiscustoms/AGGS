...
# --- [구글 시트 연결 - 직접 읽기 방식] ---
try:
    # 관세사님의 시트 ID를 직접 입력하여 CSV 형태로 읽어옵니다.
    sheet_id = "18jnI5hOd14hOxshLvdcQnPRlgj-KClT9pzc2kHqpflA"
    url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=csv&gid=0"
    df_users = pd.read_csv(url)
    
    # 만약 탭 이름이 users라면 gid 값을 확인해야 하지만, 
    # 보통 첫 번째 탭은 gid=0입니다.
except Exception as e:
    st.error(f"❌ 직접 연결 시도 실패: {e}")
    df_users = pd.DataFrame(columns=["id", "pw", "name", "is_approved", "is_admin", "access_tabs"])

# --- [2. 시스템 연결 진단 코드] --- ★ df_users 정의 이후에 배치
st.write("### 🔍 시스템 연결 진단")
try:
    st.write(f"현재 시트에서 읽어온 계정 수: {len(df_users)}개")
    if not df_users.empty:
        st.success("✅ 구글 시트 연결 성공!")
        st.write("--- 읽어온 데이터 샘플 (보안주의) ---")
        st.dataframe(df_users[['id', 'name', 'is_approved']].head(2)) # 상위 2개만 출력
    else:
        st.warning("⚠️ 시트 연결은 되었으나 데이터가 비어있습니다. 구글 시트의 'users' 탭과 헤더를 확인하세요.")
except Exception as e:
    st.error(f"❌ 진단 과정 중 에러 발생: {e}")

# --- [3. DB 설정 및 로컬 환경 구축] ---
def get_db_path():
    desktop = os.path.join(os.path.expanduser("~"), "Desktop")
    ags_dir = os.path.join(desktop, "AGS")
    if not os.path.exists(ags_dir):
        try: os.makedirs(ags_dir)
        except: return "customs_master.db" 
    return os.path.join(ags_dir, "customs_master.db")
...
if not st.session_state.logged_in:
    ...
    with cl2:
        with st.form("login_form"):
            # 입력받는 즉시 앞뒤 공백 제거
            l_id = st.text_input("아이디").strip()
            l_pw = st.text_input("비밀번호", type="password").strip()
            
            if st.form_submit_button("로그인"):
                # 입력 비밀번호의 해시값 생성
                input_pw_hash = hashlib.sha256(l_pw.encode()).hexdigest()
                
                # 구글 시트 데이터 전처리 (중요: 공백 및 데이터 형식 완전 방역)
                df_clean = df_users.copy()
                # 모든 컬럼을 문자열로 바꾸고, 앞뒤의 보이지 않는 공백(\n, \r, 스페이스)을 제거
                ...
                if not user_match.empty:
                    ...
                    if user_info['is_approved'] == '1':
                        ...
                        st.session_state.is_admin = (user_info['is_admin'] == '1')
                        st.rerun()
                    else:
                        st.error("승인 대기 중인 계정입니다.")
                else:
                    st.error("아이디 또는 비밀번호가 일치하지 않습니다.")
                    # [디버깅] 만약 계속 실패하면 아래 주석을 풀어서 해시값을 대조해 보세요
                    # st.write(f"입력 PW 해시: {input_pw_hash}")
                    # st.write(f"시트 PW 해시: {df_clean.iloc[0]['pw'] if not df_clean.empty else '데이터없음'}")
    st.stop()
...
if st.session_state.get('is_admin', False):
    with tabs[-1]:
        ...
        if not st.session_state.admin_verified:
            ...
            # --- [서브탭 2: 계정 및 권한 관리 - 구글 시트 실시간 연동] ---
            with adm_tabs[1]:
                st.subheader("👤 화주 계정 상세 제어 (Google Sheets 연동)")
                
                # 1. 신규 등록
                with st.expander("➕ 신규 화주 등록 및 구글 시트 저장"):
                    c1, c2, c3 = st.columns(3)
                    nid = c1.text_input("아이디(ID)", key="adm_reg_id")
                    nnm = c2.text_input("성명/업체명", key="adm_reg_nm")
                    npw = c3.text_input("초기 비밀번호", value="1234", type="password", key="adm_reg_pw")
                    
                    if st.button("구글 시트에 계정 추가"):
                        if nid and nnm:
                            hashed_pw = hashlib.sha256(npw.encode()).hexdigest()
                            # 신규 행 데이터 생성
                            ...
                            st.success(f"✅ {nnm} 계정이 구글 시트에 성공적으로 등록되었습니다.")
                            time.sleep(1)
                            st.rerun()
                        else: st.warning("ID와 성명을 모두 입력해주세요.")

                st.divider()
                
                # 2. 계정 목록 조회 및 제어
                st.write("**📊 현재 등록된 계정 현황 (Google Sheets 내용)**")
                if not df_users.empty:
                    # 관리자 제외 리스트 출력
                    display_df = df_users[df_users["is_admin"].astype(int) == 0]
                    st.dataframe(display_df[["id", "name", "is_approved"]], use_container_width=True, hide_index=True)
                    
                    if not display_df.empty:
                        target = st.selectbox("관리 대상 선택", display_df["id"].tolist(), key="adm_sel_target")
                        
                        col_r1, col_r2 = st.columns(2)
                        if col_r1.button("🔑 선택 계정 비밀번호 1234로 초기화"):
                            reset_pw = hashlib.sha256("1234".encode()).hexdigest()
                            # 로컬 데이터프레임 수정 후 시트 전체 업데이트 (가장 안전한 방식)
                            df_users.loc[df_users["id"] == target, "pw"] = reset_pw
                            conn_gs.update(worksheet="users", data=df_users)
                            st.info(f"💡 {target}의 비밀번호가 1234로 초기화되었습니다.")
                            
                        if col_r2.button("🗑️ 계정 영구 삭제", type="primary"):
                            # 해당 유저 제외 후 업데이트
                            df_users = df_users[df_users["id"] != target]
                            conn_gs.update(worksheet="users", data=df_users)
