...
# --- [Tab 5] 세액계산기 (과세표준 디자인 강제 고정 및 레이아웃 복구) --- #변경내역★★
with tabs[4]:
    # CSS: 13px 폰트 유지 및 과세표준 전용 최우선순위 스타일 정의
    st.markdown("""
        <style>
    ...
    with st.container(border=True):
        ...
        with c_left:
            ...
            with l_r1_1:
                p_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0, format="%.2f", key="v5_price_f")
            with l_r1_2:
            ...
            with l_r2[0]:
                s_curr = st.selectbox("통화선택", currency_options, index=0, label_visibility="collapsed", key="v5_curr_f")
                t_curr = s_curr.split(" ")[0]
            with l_r2[1]:
                e_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_mode_f")
                im_tp = "2" if e_mode == "수입" else "1"
            with l_r2[2]:
                if st.button("환율조회", use_container_width=True, key="v5_api_f"):
                    API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                    if API_KEY:
            ...
            with l_r3_1: p_frt = st.number_input("운임 (Freight, KRW)", min_value=0, key="v5_frt_f")
            with l_r3_2: p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0, key="v5_ins_f")
        ...
        with c_right:
            ...
            with h1: hs_in = st.text_input("HSK 입력", label_visibility="collapsed", key="v5_hs_f", placeholder="HS 10자리")
            with h2:
                if st.button("적용", key="v5_hbtn_f", use_container_width=True):
                    if hs_in:
            ...
            with r_r2_1: a_d = st.number_input("관세율 (%)", value=st.session_state.calc_d, key="v5_duty_f")
            with r_r2_2: a_v = st.number_input("부가세율 (%)", value=10.0, key="v5_vat_f")

            # --- [과세표준 출력부: 절대 깨지지 않는 구조] ---
            c_val = int((p_price * p_ex) + p_frt + p_ins)
            st.write("과세표준 (CIF KRW)")
            # 1. 클래스 기반 스타일링 + 2. 인라인 스타일 2중 적용
            st.markdown(f"""
    ...
    # [4] 세액 계산 버튼 및 결과 테이블
    if st.button("세액 계산 실행", use_container_width=True, type="primary", key="v5_calc_f"):
