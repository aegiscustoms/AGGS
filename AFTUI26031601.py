...
# --- [Tab 4] 통관 진행 정보 (화물 & 우편물 통합: 예외처리 강화 버전) ---
with tabs[3]:
    ...
    with st.container(border=True):
        ...
        with c_y: carg_year = st.selectbox("입항년도", [2026, 2025, 2024], index=0, key="cargo_yy_v3")
        with c_b: bl_input = st.text_input("B/L 번호", value=st.session_state.bl_val, key="bl_search_v3").strip().upper()
        with c_m: mrn_input = st.text_input("화물관리번호", value=st.session_state.mrn_val, key="mrn_search_v3").strip()
        with c_btn: 
            st.write(" ") 
            cargo_btn = st.button("화물 조회", use_container_width=True, type="primary", key="btn_cargo_v3")
    ...
    with st.container(border=True):
        ...
        with p_n: 
            psmt_no_raw = st.text_input("우편물 번호 (13자리)", placeholder="예: EE123456789KR", key="post_no_v3").strip().upper()
        with p_btn:
            st.write(" ") 
            post_btn = st.button("우편물 조회", use_container_width=True, type="primary", key="btn_post_v3")
    ...
    # --- 2. 우편물(EMS) 조회 로직 (강력한 재시도 및 디버깅 포함: 변경내역★★★★) ---
    if post_btn:
        POST_KEY = st.secrets.get("POST_API_KEY", "").strip()
        if not POST_KEY: 
            st.error("❌ 우편물 API 키(POST_API_KEY)가 설정되지 않았습니다. Secrets 설정을 확인해주세요.")
        elif not psmt_no_raw: 
            st.warning("⚠️ 우편물 번호를 입력해주세요.")
        else:
            with st.spinner("유니패스 우편물 정보 조회 중..."):
                ...
                clean_no = re.sub(r'[^a-zA-Z0-9]', '', psmt_no_raw).upper() # 번호 정제
                
                found_data = False
                error_log = []

                # EMS(01)와 일반(02) 모두 시도
                for kcd in ["01", "02"]:
                    ...
                    try:
                        ...
                        if res.status_code == 200:
                            ...
                            tcnt = root.findtext(".//tcnt") # 결과 개수 태그 확인
                            ...
                            if items and len(items) > 0:
                                ...
                                # 결과가 없을 때 API가 주는 메시지 수집
                                msg = root.findtext(".//ntceMsgCn") or "결과 없음"
                                error_log.append(f"유형[{kcd}]: {msg}")
                    except Exception as e:
                        error_log.append(f"유형[{kcd}]: 통신 오류 ({str(e)})")
                
                if found_data:
                    st.rerun()
                else:
                    # 상세 에러 원인 표시
                    st.warning("⚠️ 정보를 찾을 수 없습니다.")
                    with st.expander("상세 원인 보기"):
                        for log in error_log: st.write(log)
                        st.info("Tip: 방금 발송된 우편물은 유니패스에 등록되기까지 시간이 걸릴 수 있습니다.")
    ...
    if st.session_state.last_search_type == "CARGO" and st.session_state.cargo_result:
        ...
        if info is not None:
            st.divider()
            st.success("✅ 화물 조회 결과")
            # ... (이하 기존 화물 상세 출력 코드 생략)

    # [3-2] 우편물 출력 (응답 필드 네임스페이스 무시 파싱: 변경내역★★★★)
    if st.session_state.last_search_type == "POST" and st.session_state.post_result:
        ...
        if items:
            ...
            m1.metric("최신단계", latest.findtext("psmtPrcsTpcdNm") or "정보없음")
            m2.metric("처리기관", latest.findtext("csclPsofNm") or "정보없음")
            m3.metric("발송국가", latest.findtext("sendCntyCdNm") or "정보없음")
            m4.metric("총중량", f"{latest.findtext('ttwg') or '0'} kg")
            ...
            for item in items:
                ...
                fmt_dt = f"{dt_raw[0:4]}.{dt_raw[4:6]}.{dt_raw[6:8]}" if len(dt_raw) >= 8 else dt_raw
                post_data.append({
                    "진행단계": item.findtext("psmtPrcsTpcdNm") or "정보없음",
                    "장소(우체국)": item.findtext("csclPsofNm") or "정보없음",
                    "날짜/일시": fmt_dt,
                    "통관관리번호": item.findtext("psmtCsclMtNo") or "-"
                })
            
            # 테이블 가시성 확보를 위해 Pandas 스타일 적용
            df_p = pd.DataFrame(post_data)
            st.table(df_p)
