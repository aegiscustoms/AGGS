...
# --- [Tab 4] 통관 진행 정보 (연결 안정화 및 화물 출력 복구: 변경내역★★★★) ---
with tabs[3]:
    ...
    # --- 2. 우편물(EMS) 조회 로직 (차단 회피 로직 강화: 변경내역★★★★) ---
    if post_btn:
        ...
        if not POST_KEY: st.error("❌ 우편물 API 키(POST_API_KEY) 설정 누락.")
            ...
            with st.spinner("우편물 정보 조회 중..."):
                ...
                for kcd in ["01", "02"]:
                    try:
                        # 매 호출마다 새로운 세션으로 시도 (Connection Reset 방지)
                        ...
                        time.sleep(1.0) # 차단 방지를 위해 대기 시간 연장
                    except Exception as e:
        ...
        if info is not None:
            ...
            if dtls:
                ...
                for d in dtls:
                    ...
                    fmt_dt = f"{dt_str[0:4]}.{dt_str[4:6]}.{dt_raw[6:8]} {dt_raw[8:10]}:{dt_raw[10:12]}" if len(dt_str)>=12 else dt_str
