...
# --- [Tab 6] 관리자 모드 (최종 설계본: 계정, 로그, API 점검) --- #변경내역★★
if st.session_state.is_admin:
    with tabs[-1]:
        st.markdown("<div class='custom-header'>⚙️ 시스템 관리자 센터</div>", unsafe_allow_html=True)

        # 1. 관리자 2차 인증 설정 (세션 유지)
        if "admin_verified" not in st.session_state:
            st.session_state.admin_verified = False

        if not st.session_state.admin_verified:
            with st.container(border=True):
                st.write("🔒 **보안을 위해 관리자 암호를 한 번 더 입력해주세요.**")
                a_pw = st.text_input("Admin Password", type="password", key="admin_re_auth")
                if st.button("관리자 모드 진입", use_container_width=True):
                    if a_pw == "admin1234":  # 필요 시 st.secrets에서 관리 권장
            ...
            # 관리자 전용 메뉴 구성
            st.info("✅ 관리자 인증 완료. 시스템 설정 및 로그 확인이 가능합니다.")
            
            a_tabs = st.tabs(["👤 사용자 및 권한 관리", "📋 이용 로그 확인", "🔌 API 상태 점검 도구"])
            ...
            with a_tabs[0]:
                ...
                try:
                    conn = sqlite3.connect("customs_master.db")
                    # 사용자 테이블 조회 (테이블이 없을 경우 대비 예외처리)
                    u_df = pd.read_sql("SELECT username, name, role, last_login FROM users", conn)
                    st.dataframe(u_df, use_container_width=True)
                    
                    with st.expander("➕ 신규 화주/사용자 계정 생성"):
                        c1, c2 = st.columns(2)
                        new_uid = c1.text_input("아이디 (ID)")
                        new_unm = c2.text_input("성명/업체명")
                        new_upw = c1.text_input("초기 비밀번호", type="password")
                        new_url = c2.selectbox("권한 설정", ["USER", "ADMIN"])
                        
                        if st.button("계정 생성 실행", use_container_width=True):
                            # 실제 DB INSERT 로직 구현 위치
                            st.success(f"✅ {new_unm} 계정이 성공적으로 생성되었습니다.")
                    conn.close()
                except:
                    st.warning("⚠️ 사용자 데이터베이스 구조를 확인 중입니다. (users 테이블 미생성)")

            # --- [B] 접속 및 이용 로그(Log) 확인 ---
            with a_tabs[1]:
                st.subheader("📋 시스템 이용 로그")
                # 실제 운영 시 DB의 로그 테이블에서 가져오며, 여기선 시각화를 위해 샘플 배치
                log_sample = [
                    {"일시": "2026-03-18 16:45:12", "사용자": "관세사님", "활동": "API 점검", "내용": "환율 API 호출 테스트"},
                    {"일시": "2026-03-18 16:30:05", "사용자": "화주A", "활동": "세액계산", "내용": "HS 8517.13-0000 조회"},
                    {"일시": "2026-03-18 15:12:44", "사용자": "화주B", "활동": "화물조회", "내용": "운송장번호 6093... 조회"}
                ]
                st.table(log_sample)
                
                c1, c2 = st.columns([1, 4])
                if c1.button("📥 로그 내보내기"):
                    st.toast("로그 데이터가 CSV로 준비되었습니다.")

            # --- [C] API 상태 점검 도구 (인증키 유효성 진단) ---
            with a_tabs[2]:
                st.subheader("🔌 유니패스 API 실시간 상태 점검")
                st.write("인증키가 유효한지, 관세청 서버 응답이 정상인지 즉시 테스트합니다.")
                ...
                with check_c1:
                    with st.container(border=True):
                        st.markdown("**💰 관세환율 API**")
                        if st.button("환율 API 핑 테스트", use_container_width=True):
                            ...
                            try:
                                ...
                                if "존재하지 않는 인증키" in res.text:
                                    st.error("❌ 미승인 키: 유니패스 승인 필요")
                                elif res.status_code == 200:
                                    st.success("✅ 정상: 데이터 수신 완료")
                                else:
                                    st.warning(f"⚠️ 오류: HTTP {res.status_code}")
                            except:
                                st.error("❌ 통신불가: 네트워크 확인")

                # 2. 우편물조회 API 점검 (API037)
                with check_c2:
                    with st.container(border=True):
                        st.markdown("**📦 우편물조회 API**")
                        if st.button("우편물 API 핑 테스트", use_container_width=True):
            ...
            st.divider()
            if st.button("🔴 관리자 모드 종료 (로그아웃)", use_container_width=True):
                st.session_state.admin_verified = False
                st.rerun()
