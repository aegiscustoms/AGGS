...
# --- [Tab 4] 통관 진행 정보 (오류 수정 및 연결 안정화: 변경내역★★★★) ---
with tabs[3]:
    ...
    with st.container(border=True):
        ...
        with c_y: carg_year = st.selectbox("입항년도", [2026, 2025, 2024], index=0, key="cargo_yy_final_v5")
        with c_b: bl_input = st.text_input("B/L 번호", value=st.session_state.bl_val, key="bl_search_v5").strip().upper()
        with c_m: mrn_input = st.text_input("화물관리번호", value=st.session_state.mrn_val, key="mrn_search_v5").strip()
        with c_btn: 
            st.write(" ") 
            cargo_btn = st.button("화물 조회", use_container_width=True, type="primary", key="btn_cargo_v5")
    ...
    with st.container(border=True):
        ...
        with p_n: 
            psmt_no_raw = st.text_input("우편물 번호 (13자리)", placeholder="예: EE123456789KR", key="post_no_v5").strip().upper()
        with p_btn:
            st.write(" ") 
            post_btn = st.button("우편물 조회", use_container_width=True, type="primary", key="btn_post_v5")

    # --- 1. 일반화물 조회 로직 ---
    if cargo_btn:
        API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
        if not API_KEY: st.error("❌ API 키(UNIPASS_API_KEY)가 설정되지 않았습니다.")
        else:
            with st.spinner("관세청 화물 데이터 조회 중..."):
                ...
                try:
                    res = requests.get(url, params=params, timeout=20) # 타임아웃 연장
                    if res.status_code == 200:
    ...
    # --- 2. 우편물(EMS) 조회 로직 (연결 안정화 및 재시도 로직 보강) ---
    if post_btn:
        POST_KEY = st.secrets.get("POST_API_KEY", "").strip()
        if not POST_KEY: st.error("❌ 우편물 API 키(POST_API_KEY) 설정이 필요합니다.")
            ...
            with st.spinner("우편물 정보 조회 중..."):
                ...
                for kcd in ["01", "02"]:
                    try:
                        ...
                        # Connection Reset 방지를 위해 매번 세션을 새로 생성하거나 타임아웃을 넉넉히 줌
                        ...
                        time.sleep(1.0) # 호출 간격을 1초로 늘려 서버 차단 방지
                    except Exception as e: 
                ...
                if found_data: st.rerun()
                    ...
                    with st.expander("상세 원인 보기"):
                        for log in error_log: st.write(log)

    # --- 3. 결과 출력 영역 ---
    # [3-1] 일반화물 출력 (NameError 오타 수정 및 상세 출력 복구)
    if st.session_state.last_search_type == "CARGO" and st.session_state.cargo_result:
        ...
        if info is not None:
            ...
            m1.metric("현재상태", info.findtext("prgsStts") or "정보없음")
            m2.metric("품명", (info.findtext("prnm") or "정보없음")[:12])
            ...
            if dtls:
                ...
                th_style = 'style="background-color:#F8FAFC; color:#1E3A8A; padding:10px; border:1px solid #E2E8F0; font-size:13px; text-align:center;"'
                ...
                for item in dtls:
                    dt_str = item.findtext("prcsDttm") or ""
                    # [수정] dt_raw -> dt_str로 변수명 오타 수정
                    f_dt = f"{dt_str[0:4]}.{dt_str[4:6]}.{dt_str[6:8]} {dt_str[8:10]}:{dt_str[10:12]}" if len(dt_str)>=12 else dt_str
                    html += f'<tr><td {td_style}>{item.findtext("cargTrcnRelaBsopTpcd") or ""}</td>'
                    html += f'<td {td_style}>{f_dt}</td>'
                    html += f'<td {td_style}>{item.findtext("shedNm") or item.findtext("rlbrCn") or ""}</td></tr>'
                html += '</tbody></table>'
                st.write(f'<div style="max-height:400px; overflow-y:auto; border:1px solid #E2E8F0; border-radius:5px;">{html}</div>', unsafe_allow_html=True)
    ...
    if st.session_state.last_search_type == "POST" and st.session_state.post_result:
        ...
        if items:
            ...
            top = items[0]
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("처리단계", top.findtext("psmtPrcsTpcdNm") or "-")
            m2.metric("우체국", top.findtext("csclPsofNm") or "-")
            m3.metric("발송국", top.findtext("sendCntyCdNm") or "-")
            m4.metric("총중량", f"{top.findtext('ttwg') or '0'} kg")
