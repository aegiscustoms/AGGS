    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 관세청 통계 조회 및 AI 전문 분석 중..."):
                try:
                    # [2-1] 관세청 HS CODE 내비게이션 API 호출 로직 #변경내역★★
                    stats_data = "관세청 신고 통계 정보를 불러올 수 없습니다."
                    NAVI_KEY = st.secrets.get("HSNAVI_API_KEY", "")
                    ...
                    if NAVI_KEY and p_name:
                        ...
                        # 품명의 앞글자를 활용하여 검색 성능 최적화
                        params = {"crkyCn": NAVI_KEY, "hsSgn": p_name[:4]} 
                        ...
                        try:
                            ...
                            if res.status_code == 200:
                                ...
                                if items:
                                    stats_list = []
                                    for idx, item in enumerate(items[:3]): # 상위 3개 추출
                                        h_code = item.findtext("hs10Sgn")
                                        h_name = item.findtext("prlstNm")
                                        rank = item.findtext("acrsTcntRnk")
                                        stats_list.append(f"- {idx+1}위: {h_code} ({h_name[:30]}...) / 실적순위: {rank}위")
                                    stats_data = "\n".join(stats_list)
                        except: pass
