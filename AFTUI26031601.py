...
# --- [Tab 1] HS검색 (관세청 통계 연동 및 서식 고도화 버전) --- #변경내역★★
with tabs[0]:
    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 관세청 통계 조회 및 AI 전문 분석 중..."):
                try:
                    # [2-1] 관세청 HS CODE 내비게이션 API 호출 로직 (보정형) #변경내역★★
                    stats_data = "조회된 통계 정보가 없습니다."
                    ...
                    if NAVI_KEY and p_name:
                        ...
                        # 서버 차단 방지를 위한 브라우저 헤더 설정
                        headers = {
                            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
                        }
                        
                        # 1차 시도: 품명(prlstNm)으로 직접 검색
                        params = {
                            "crkyCn": NAVI_KEY,
                            "prlstNm": p_name
                        } 
                        ...
                        try:
                            # headers를 추가하여 접속 거부(Connection Reset) 방지
                            ...
                            if res.status_code == 200:
                                ...
                                # 품명 검색 결과가 없을 경우 2차 시도: HS코드 앞자리 검색
                                if not items and len(p_name) >= 2:
                                    params_hs = {"crkyCn": NAVI_KEY, "hsSgn": p_name[:4]}
                                    res = requests.get(url, params=params_hs, headers=headers, timeout=10)
                                    root = ET.fromstring(res.text)
                                    items = root.findall(".//cmtrStatsQryRsltVo")
                                ...
                                if items:
                                    ...
                                    for idx, item in enumerate(items[:3]):
                                        h_code = item.findtext("hs10Sgn") # HS 10단위
                                        h_name = item.findtext("prlstNm") # 품명
                                        rank = item.findtext("acrsTcntRnk") # 실적순위
                                        if h_code:
                                            stats_list.append(f"{idx+1}위: {h_code} ({h_name}) - 실적순위 {rank}위")
                                    
                                    if stats_list:
                                        stats_data = "\n".join(stats_list)
                            else:
                                stats_data = f"통계 조회 실패 (HTTP {res.status_code})"
                                
                        except Exception as api_err:
                            # 에러 타입과 메시지를 상세히 출력하여 자가진단 지원
                            stats_data = f"통계 조회 중 오류 발생: {type(api_err).__name__} ({str(api_err)})"

                    # [2-2] 제미나이 분석 (통계 데이터 포함)
                    model = genai.GenerativeModel('gemini-2.0-flash')
...
- 관련 부/류의 주(Note) 규정과 관세율표 해설서 내용을 검토할 것.
- 통칙 제1호(호의 용어 및 주 규정)를 최우선 적용하고, 제2호부터 제6호까지 논리적으로 순차 적용할 것.
- "4. 예상 경합세번"은 추천 번호의 확률이 100%가 아닌 경우만 표시하고, 상위 3순위까지 추천할 것.

[AI 분석 리포트 작성 지침]
- 리포트 내 항목명과 항목내용 사이에는 줄바꿈을 하지 말고, 이전 항목 내용과 다음 항목명 사이에는 반드시 빈 줄을 추가하세요.
- 각 항목명(1., 2., 3., 4.)은 **굵게** 표시하세요.
- 하위 정보의 글머리는 "1)", "-", "•" 중 하나를 사용하세요.
- 아래 특정 문구는 HTML 태그를 사용하여 크기 및 색상을 조절하세요:
  1) "💡 AI 분석 리포트" -> 20px
