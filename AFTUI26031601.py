...
# --- [Tab 5] 세액계산기 (UI 통합 및 폰트 13px 최적화) --- #변경내역★★
with tabs[4]:
    # CSS: 탭 내 모든 라벨 및 입력창 폰트 사이즈 조정
    st.markdown("""
        <style>
            .v5-font-style label, .v5-font-style input, .v5-font-style div {
    ...
    # 58개국 리스트
    currency_options = [
        "USD (미국)", "CNY (중국)", "JPY (일본)", "EUR (유럽연합)", "GBP (영국)", "CAD (캐나다)",
        "AUD (호주)", "HKD (홍콩)", "SGD (싱가포르)", "VND (베트남)", "THB (태국)", "IDR (인도네시아)",
        "INR (인도)", "AED (아랍에미리트)", "CHF (스위스)", "NZD (뉴질랜드)", "MYR (말레이시아)",
        "PHP (필리핀)", "SAR (사우디아라비아)", "TWD (대만)", "DKK (덴마크)", "NOK (노르웨이)",
        "SEK (스웨덴)", "RUB (러시아)", "ZAR (남아프리카공화국)", "MXN (멕시코)", "BRL (브라질)",
        "ILS (이스라엘)", "JOD (요르단)", "KWD (쿠웨이트)", "BHD (바레인)", "OMR (오만)",
        "TRY (터키)", "CZK (체코)", "PLN (폴란드)", "HUF (헝가리)", "RON (루마니아)",
        "EGP (이집트)", "PKR (파키스탄)", "BDT (방글라데시)", "LKR (스리랑카)", "MMK (미얀마)",
        "KHR (캄보디아)", "MNT (몽골)", "KZT (카자흐스탄)", "UZS (우즈베키스탄)", "ARS (아르헨티나)",
        "CLP (칠레)", "COP (콜롬비아)", "PEN (페루)", "QAR (카타르)", "DZD (알제리)",
        "NGN (나이지리아)", "KES (케냐)", "TZS (탄자니아)", "GHS (가나)", "ZMW (잠비아)", "KRW (한국)"
    ]

    with st.container(border=True):
        st.markdown("<div class='v5-font-style'><b>📍 1. 과세가격(CIF) 및 품목 입력</b></div>", unsafe_allow_html=True)
        
        # 메인 영역 분할
        cl, cr = st.columns(2)
        
        with cl:
            st.markdown("<div class='v5-font-style'>", unsafe_allow_html=True)
            p_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0, format="%.2f", key="v5_inv_p")
            p_frt = st.number_input("운임 (Freight, KRW)", min_value=0, key="v5_frt_p")
            p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0, key="v5_ins_p")
            st.markdown("</div>", unsafe_allow_html=True)
            
        with cr:
            st.markdown("<div class='v5-font-style'>", unsafe_allow_html=True)
            
            # --- 적용환율 + API 조회 통합 라인 (수정사항 1, 2 반영) ---
            st.write("적용환율 및 조회")
            ex_row = st.columns([1, 1.5, 1, 1])
            
            with ex_row[0]: # 적용환율 입력창 (1/4 크기)
                p_ex = st.number_input("적용환율", label_visibility="collapsed", value=st.session_state.api_ex_rate, format="%.2f", key="v5_p_ex")
            
            with ex_row[1]: # 국가/통화코드 선택
                sel_curr = st.selectbox("통화선택", currency_options, index=0, label_visibility="collapsed", key="v5_curr_sel")
                target_curr = sel_curr.split(" ")[0]
                
            with ex_row[2]: # 조회 버튼
                # 환율구분 간단히 (수입/수출)
                ex_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_ex_mode")
                imex_tp = "2" if ex_mode == "수입" else "1"
                
                if st.button("환율조회", use_container_width=True, key="v5_api_btn"):
                    API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                    if API_KEY:
                        with st.spinner(""):
                                ...
                                if res.status_code == 200:
                                    ...
                                    for item in items:
                                        if item.findtext("currSgn") == target_curr:
                                            ...
                                            found = True; break
                                    if not found: st.session_state.api_ex_disp = "N/A"
                            except: st.session_state.api_ex_disp = "Error"
                        st.rerun()

            with ex_row[3]: # 결과 출력
                disp_text = st.session_state.api_ex_disp if st.session_state.api_ex_disp else "조회결과"
                st.markdown(f"<div style='background-color:#F1F5F9; padding:7px; border-radius:4px; text-align:center; font-size:13px; font-weight:bold; height:38px; line-height:24px; border:1px solid #CBD5E1;'>{disp_text}</div>", unsafe_allow_html=True)
            
            # --- HS코드 및 세율 영역 ---
            st.write("품목분류(HSK)")
            h1, h2 = st.columns([0.7, 0.3])
            hs_in = h1.text_input("HSK 입력", label_visibility="collapsed", key="v5_hs_input", placeholder="HS 10자리")
            if h2.button("적용", key="v5_hs_apply"):
                if hs_in:
            ...
            r1, r2 = st.columns(2)
            a_d = r1.number_input(f"관세율({st.session_state.calc_t}, %)", value=st.session_state.calc_d, key="v5_duty")
            a_v = r2.number_input("부가세율(%)", value=10.0, key="v5_vat")
            st.markdown("</div>", unsafe_allow_html=True)
        
        cif = int((p_price * p_ex) + p_frt + p_ins)
        st.markdown(f"<div style='font-size:13px; color:#1E40AF; font-weight:bold; margin-top:10px;'>✔️ 과세표준 (CIF KRW): {cif:,.0f} 원</div>", unsafe_allow_html=True)
        
    # [4] 세액 계산 실행
    if st.button("세액 계산 실행", use_container_width=True, type="primary", key="v5_main_exec"):
        d = int(cif * (a_d/100))
        v = int((cif + d) * (a_v/100))
        
        st.markdown(f"<div style='font-size: 20px; font-weight: bold; color: #B91C1C; text-align: right; background-color: #FEF2F2; padding: 12px; border-radius: 8px; margin-bottom:15px;'>💰 예상세액 합계: {d+v:,.0f} 원</div>", unsafe_allow_html=True)
        ...
        td_center = 'style="text-align:center; padding:8px; border:1px solid #F1F5F9; font-size:13px;"'
        td_right = 'style="text-align:right; padding:8px; border:1px solid #F1F5F9; font-size:13px; font-weight:600;"'
        
        html = f"""
        <table {t_style}>
            <thead><tr><th {th_style}>세종</th><th {th_style}>산출근거</th><th {th_style}>세액(원)</th></tr></thead>
            <tbody>
                <tr><td {td_center}>관세</td><td {td_center}>{cif:,.0f} x {a_d}%</td><td {td_right}>{d:,.0f}</td></tr>
                <tr><td {td_center}>부가세</td><td {td_center}>({cif:,.0f} + {d:,.0f}) x {a_v}%</td><td {td_right}>{v:,.0f}</td></tr>
            </tbody>
        </table>
        """
    ...
    st.markdown("""
        <div style="font-size: 12px; color: #64748B; margin-top: 20px; line-height: 1.6; border-top: 1px solid #E2E8F0; padding-top: 10px;">
            ※ 관세청 API 실시간 고시환율을 참조합니다. (수입/수출 구분 선택 후 조회)<br>
            ※ 예상세액은 참조용이며 실제 납부세액과 차이가 있을 수 있습니다.
        </div>
    """, unsafe_allow_html=True)
