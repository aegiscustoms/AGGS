...
# --- [Tab 6] 관리자 모드 (데이터 업로드 + 계정/권한 제어 + 점검모드 + 핑테스트 통합) ---
    ...
    with tabs[-1]:
        ...
        if not st.session_state.admin_verified:
            ...
            with adm_tabs[0]:
                ...
                st.subheader("📁 HS 마스터 및 요건 데이터 관리")
                ...
                for i, m_name in enumerate(m_list):
                    with cols[i%3]:
                        ...
                        if up and st.button(f"반영", key=f"btn_{m_name}_f", use_container_width=True):
                            # (기존 업로드/SQLAlchemy 로직 수행 부분...)
                            st.session_state.upload_status[m_name] = True
                            st.success(f"✅ {m_name} 반영 완료")
                            st.rerun()

            # --- [서브탭 2: 계정 및 탭별 권한 상세 관리] ---
            with adm_tabs[1]:
                st.subheader("👤 화주 계정 상세 제어")
                
                # 1. 신규 계정 생성 및 탭 권한 부여
                with st.expander("➕ 신규 화주 등록 및 탭 권한 설정", expanded=False):
                    c1, c2 = st.columns(2)
                    ...
                    is_paid = st.checkbox("유료 회원(Premium) 여부", key="paid_check")
                    
                    if st.button("계정 생성 및 권한 부여 실행"):
                        if new_id and new_nm:
                            st.success(f"✅ {new_nm}({new_id}) 계정이 생성되었습니다. (허용: {access_tabs})")
                        else:
                            st.warning("⚠️ 아이디와 성명을 입력해주세요.")
                ...
                try:
                    ...
                    st.write("**🗑️ 계정 강제 삭제**")
                    target_del = st.selectbox("삭제 대상 선택", u_df["id"].tolist() if not u_df.empty else ["없음"])
                    if st.button("선택한 계정 영구 삭제", type="primary"):
                        st.warning(f"⚠️ {target_del} 계정이 시스템에서 삭제되었습니다.")
                    conn.close()
                except:
                    st.info("💡 사용자 데이터베이스가 비어있거나 연결되지 않았습니다.")

            # --- [서브탭 3: 이용 로그 확인] ---
            with adm_tabs[2]:
                st.subheader("📋 실시간 시스템 이용 로그")
                log_data = [
                    {"일시": "2026-03-19 14:20", "ID": "user123", "업체명": "이지스물류", "활동": "HS코드 조회"},
                    {"일시": "2026-03-19 15:05", "ID": "customs_pro", "업체명": "화주A", "활동": "우편물 API 조회"},
                ]
                st.table(log_data)

            # --- [서브탭 4: API 연결 상태 핑테스트 보강] ---
            with adm_tabs[3]:
                st.subheader("🔌 유니패스 API 실시간 연결 진단 (Ping)")
                ck1, ck2, ck3 = st.columns(3)
                
                with ck1:
                    with st.container(border=True):
                        st.write("💰 **관세환율**")
                        if st.button("핑 테스트", key="ping_exch"):
                            ...
                            try:
                                ...
                                if "존재하지 않는 인증키" in res.text: st.error("❌ 미승인")
                                elif res.status_code==200: st.success("✅ 정상")
                            except: st.error("❌ 연결실패")

                with ck2:
                    with st.container(border=True):
                        st.write("📦 **우편물조회**")
                        if st.button("핑 테스트", key="ping_post"):
                            K = st.secrets.get("POST_API_KEY", "").strip()
                            # 샘플 번호로 테스트
                            URL = f"https://unipass.customs.go.kr:38010/ext/rest/psmtCsclPrgsInfoQry/retrievePsmtCsclPrgsInfo?crkyCn={K}&psmtKcd=14&psmtNo=EE123456789KR"
                            try:
                                res = requests.get(URL, timeout=5)
                                if "존재하지 않는 인증키" in res.text: st.error("❌ 미승인")
                                elif res.status_code==200: st.success("✅ 정상")
                ...
                with ck3:
                    with st.container(border=True):
                        st.write("🚛 **화물진행**")
                        if st.button("핑 테스트", key="ping_cargo"):
                            K = st.secrets.get("UNIPASS_API_KEY", "").strip()
                            # 샘플 MBL로 테스트
                            URL = f"https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry/retrieveCargCsclPrgsInfo?crkyCn={K}&blYy=2026&hblNo=TEST1234"
                            try:
                                res = requests.get(URL, timeout=5)
                                if "존재하지 않는 인증키" in res.text: st.error("❌ 미승인")
                                elif res.status_code==200: st.success("✅ 정상")
...
# --- [앱 메인 로직 상단 반영 필요] 점검 모드 가로채기 ---
if st.session_state.get('maintenance_mode', False):
    if not st.session_state.get('is_admin', False):
        st.warning("🚧 시스템 점검 중입니다. 이용에 불편을 드려 죄송합니다. ")
        st.stop() # 관리자가 아니면 앱 실행 중단
