...
# --- [197행 시작] 로그인 통과 후 데이터 복구 및 사이드바 통합 제어 ---
try:
    conn_gs = st.connection("gsheets", type=GSheetsConnection)
    df_users = conn_gs.read(spreadsheet=st.secrets["connections"]["gsheets"]["spreadsheet"], worksheet="users", ttl=0)
except:
...
# ★오류 해결 핵심: 사이드바 정보 및 로그아웃 버튼을 여기서 '단 한 번만' 선언합니다.
st.sidebar.markdown(f"### 👤 {st.session_state.user_id} ({st.session_state.user_name})")

# 중복 ID 충돌 방지를 위해 key="main_logout"을 추가했습니다.
if st.sidebar.button("로그아웃", key="main_logout"): 
    st.session_state.logged_in = False
    # 쿠키가 존재할 경우에만 삭제 시도
    try:
