...
# ★변경내역★시작★ [로그인 가림막 및 잔상 차단 로직]
if 'logged_in' not in st.session_state: 
    st.session_state.logged_in = False

# [핵심] 로그인 전용 빈 컨테이너 선언 (하단 코드가 미리 보이는 것을 방지하는 가림막 역항)
login_screen = st.empty()

if not st.session_state.logged_in:
    # 로그인이 안 된 상태라면 login_screen이라는 주머니 안에서만 화면을 구성함
    with login_screen.container():
        ...
        with cl2:
            with st.form("login_form"):
                ...
                if submit:
                    # 구글 시트 통신(로그 기록 등) 중 코드 노출 방지를 위한 스피너 가림막
                    with st.spinner("본인 인증 확인 중..."):
                        # 1) 마스터 관리자 체크
    ...
    # [방어막] 로그인이 안 된 상태라면 여기서 코드 실행을 즉시 중단 (하단 탭/코드 노출 방지)
    st.stop()
# ★변경내역★끝★
