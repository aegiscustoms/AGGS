...
# --- [Tab 1] AI 기반 HS 추천 시스템 ---
with tabs[0]:
    # 1) 타이틀 섹션 (미니멀 스타일)
    st.markdown(f"""
        <div style='padding: 10px 0px; margin-bottom: 20px;'>
            <h2 style='color: #1E3A8A; font-weight: 700; letter-spacing: -1px; margin: 0;'>AI 기반 HS 추천 시스템</h2>
        </div>
    """, unsafe_allow_html=True)
    ...
    # [1] 정보 입력 구역 (좌우 2분할 레이아웃)
    with st.container(border=True):
        col_left, col_right = st.columns([0.5, 0.5], gap="large")
        
        with col_left:
            st.markdown("<p style='font-size: 16px; font-weight: 600; color: #334155;'>물품 이미지</p>", unsafe_allow_html=True)
            ...
            if uploaded_file:
                ...
                st.info("시각적 분석이 필요한 경우 이미지를 업로드하십시오.")
        
        with col_right:
            st.markdown("<p style='font-size: 16px; font-weight: 600; color: #334155;'>상세 물품 명세</p>", unsafe_allow_html=True)
    ...
    # [2] 분석 실행 로직 (버튼 스타일 커스텀)
    st.markdown("""
        <style>
        div[data-testid="stButton"] > button[key="hs_analysis_btn"] {
        ...
        div[data-testid="stButton"] > button[key="hs_analysis_btn"]:hover {
            background-color: #152963 !important;
        }
        </style>
    """, unsafe_allow_html=True)

    if st.button("AI 전문 분석 리포트 생성", key="hs_analysis_btn", use_container_width=True, type="primary"):
        details = []
        ...
        if not uploaded_file and not input_summary:
            st.warning("분석을 위해 최소 하나 이상의 정보를 입력해주십시오.")
        else:
            with st.spinner("품목 정보를 정밀 분석 중입니다..."):
                try:
    ...
    if st.session_state.ai_report_done:
        # 결과 리포트 박스 (미니멀 화이트 배경)
        st.markdown(f"""
            <div style='background-color: #ffffff; padding: 25px; border-radius: 4px; border: 1px solid #E2E8F0; margin-top: 10px;'>
                {st.session_state.last_report_text}
            </div>
        """, unsafe_allow_html=True)
        
        st.markdown("<div style='margin-top: 30px;'></div>", unsafe_allow_html=True)
        st.markdown("<h4 style='color: #334155; font-size: 16px; font-weight: 700;'>전문 관세사 추가 검토 의뢰</h4>", unsafe_allow_html=True)
        
        # 의뢰 영역 버튼 스타일 (테두리 제거 및 네이비)
        st.markdown("""
            <style>
            div[data-testid="stButton"] > button[key="footer_req_btn"] {
        ...
        with col_u1: u_org = st.text_input("상호명 (또는 성함)", key="u_org_v4", placeholder="상호를 입력하십시오.")
        with col_u2: u_contact = st.text_input("연락처 (전화/이메일)", key="u_contact_v4", placeholder="연락처를 입력하십시오.")
        with col_u3: 
            st.markdown("<div style='margin-top: 0px;'></div>", unsafe_allow_html=True)
            send_click = st.button("관세사 검토의뢰", key="footer_req_btn", use_container_width=True)
        
        if send_click:
            if not u_org or not u_contact: 
                st.error("상호명과 연락처를 입력해주십시오.")
            else:
                import smtplib
                ...
                if not S_PASS: st.error("메일 서버 설정 오류")
                else:
                    msg = MIMEMultipart()
                    msg['From'] = S_EMAIL
                    msg['To'] = S_EMAIL 
                    ...
                    try:
                        ...
                        st.success(f"검토 의뢰가 성공적으로 접수되었습니다.")
