...
# ==========================================
# [완전 통합] 탭 메뉴 및 우측 상단 유저정보 (겹침 해결판)
# ==========================================
...
if st.session_state.get('logged_in', False):
    st.markdown(f"""
        <style>
            ...
            /* 탭 메뉴 위치 조정 (상단바와 수평 맞춤) */
            .stTabs {{ margin-top: -32px; position: relative; z-index: 10; }}

            /* 유저 정보 컨테이너 (우측 상단 절대 위치) */
            .fixed-top-container {{
                position: absolute;
                top: -42px; 
                right: 0px;
    ...
    # 실제 로그아웃 버튼 (컬럼을 쓰지 않고 바로 배치하여 CSS가 제어하게 함)
    # 버튼 위치를 우측 상단 공간으로 강제 이동시키기 위한 래퍼
    st.write(f"""
        <div style="position: absolute; top: -43px; right: 0px; z-index: 1002;">
    """, unsafe_allow_html=True)
