...
# 3. [최종 교정] 탭 메뉴 라인에 유저 정보(상호명/로그아웃) 강제 우측 정렬
if st.session_state.logged_in:
    # 컬럼 대신 하나의 컨테이너 안에 flexbox를 사용하여 겹침 방지 및 우측 정렬
    st.markdown(f"""
        <div style="display: flex; justify-content: flex-end; align-items: center; gap: 10px; margin-bottom: -45px; position: relative; z-index: 999;">
    ...
    # 실제 로그아웃 버튼 (위 flexbox의 빈 공간에 절대 위치로 배치)
    _, col_logout = st.columns([9.18, 0.82]) # 비율을 조절하여 위 빈 공간에 정확히 안착
    with col_logout:
        st.write("<div style='margin-top: 5px;'>", unsafe_allow_html=True)
