...
# 3. [최종 교정] 겹침 원천 차단: HTML 절대 위치 지정 방식
if st.session_state.get('logged_in', False):
    st.markdown(f"""
        <style>
            ...
            /* 유저명 박스: 우측 상단 절대 위치 고정 */
            .fixed-user-tag {{
                position: absolute;
                top: -42px; 
                right: 95px; /* Logout 버튼 왼쪽 옆에 위치하도록 간격 확보 */
                background-color: #f8f9fa;
                padding: 4px 12px;
                border-radius: 12px;
                border: 1px solid #e2e8f0;
            ...
            /* Logout 버튼 강제 위치 고정 (stButton 내부 스타일 제어) */
            div[data-testid="stButton"] > button[key="top_logout_btn"] {{
        ...
        <div style="position: relative; width: 100%;">
            <div class="fixed-user-tag">{st.session_state.user_name}님</div>
        </div>
    """, unsafe_allow_html=True)

    # 실제 로그아웃 버튼 (CSS에서 위치를 강제했으므로 위치 상관없이 우측 상단으로 이동함)
