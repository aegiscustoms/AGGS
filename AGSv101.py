...
with tabs[0]:
    # 1. 상단 타이틀 섹션 (전문 서비스 느낌의 헤더)
    st.markdown(f"""
        <div style="background-color: #ffffff; padding: 10px 0px; border-bottom: 3px solid #1E3A8A; margin-bottom: 25px;">
            <h2 style="color: #1E3A8A; margin-bottom: 5px; font-weight: 800; letter-spacing: -1px;">
                🔍 AI 기반 HS Code 전문 분석 시스템
            </h2>
            <p style="color: #64748B; font-size: 14px; margin: 0;">
                최신 관세율표 지식과 AI 기술을 결합하여 법적 근거에 기반한 최적의 세번을 제안합니다.
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    # [1] 물품 정보 입력 구역 (카드 레이아웃 적용)
    with st.container(border=True):
        st.markdown("""
            <div style="margin-bottom: 15px;">
                <span style="background-color: #1E3A8A; color: white; padding: 3px 10px; border-radius: 4px; font-size: 12px; font-weight: bold;">INPUT</span>
                <span style="margin-left: 10px; font-weight: bold; color: #334155;">물품 정보 분석 의뢰</span>
            </div>
        """, unsafe_allow_html=True)
        ...
        with col_left:
            st.markdown("<p style='font-size:14px; font-weight:600; color:#475569; margin-bottom:8px;'>📸 물품 이미지</p>", unsafe_allow_html=True)
            ...
            if uploaded_file:
                ...
                st.info("이미지를 업로드하면 이미지 분석이 병행됩니다.")
        
        with col_right:
            st.markdown("<p style='font-size:14px; font-weight:600; color:#475569; margin-bottom:8px;'>📝 상세 물품 정보</p>", unsafe_allow_html=True)
            # 입력폼 2단 정렬로 정돈된 느낌 부여
            ...
            with sub_col1:
                ...
                p_composition = st.text_input("7) 구성요소 (Set 여부)", key="p_composition", placeholder="예: 본체+케이블 세트")
            with sub_col2:
                ...
                p_spec = st.text_input("6) 규격 및 사양", key="p_spec", placeholder="예: 220V, 15인치, ISO 인증")
    ...
    if st.session_state.ai_report_done:
        # 리포트 박스 디자인 (그림자 효과)
        st.markdown(f"""
            <div style="background-color: #ffffff; padding: 30px; border-radius: 12px; border: 1px solid #E2E8F0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); margin-top: 20px;">
        ...
        # 전문 관세사 검토 의뢰 섹션 (디자인 고도화)
        st.markdown("""
            <div style="background-color: #F8FAFC; padding: 20px; border-radius: 10px; border: 1px dashed #1E3A8A;">
                <h4 style="color: #1E3A8A; margin-top: 0; font-size: 17px;">📧 전문 관세사 추가 검토의뢰</h4>
                <p style="font-size: 13.5px; color: #64748B; margin-bottom: 0;">AI 분석 결과를 바탕으로 전문 관세사의 확정적 판단이 필요하신 경우 의뢰해 주세요.</p>
            </div>
            <style>
                div[data-testid="stButton"] > button { height: 42px !important; background-color: #1E3A8A !important; color: white !important; font-weight: bold; border-radius: 6px; }
            </style>
        """, unsafe_allow_html=True)
        
        st.markdown("<div style='margin-top: 20px;'></div>", unsafe_allow_html=True)
        col_u1, col_u2, col_u3 = st.columns([0.37, 0.37, 0.26])
        ...
        if send_click:
            if not u_org or not u_contact: st.error("⚠️ 정보를 입력해주세요.")
                ...
                if not S_PASS: st.error("❌ 이메일 전송 비번이 설정되지 않았습니다.")
                else:
                    msg = MIMEMultipart()
                    msg['From'] = S_EMAIL
                    msg['To'] = S_EMAIL 
                    ...
                    try:
                        ...
                        server.sendmail(S_EMAIL, S_EMAIL, msg.as_string())
                        server.quit()
