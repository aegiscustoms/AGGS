        ...
        if not st.session_state.admin_verified:
            ...
            with adm_tabs[1]:
                ...
                with st.expander("➕ 신규 고객사 직접 등록"):
                    c1, c2, c3, c4 = st.columns(4) # 컬럼 4개로 확장
                    nid = c1.text_input("아이디 (사업자번호)").replace("-", "")
                    nnm = c2.text_input("업체명")
                    npw = c3.text_input("초기 비밀번호", value="1234", type="password")
                    nem = c4.text_input("이메일 (선택)") # 이메일 필드 추가
                    ...
                    if st.button("고객사 등록 실행", use_container_width=True):
                        if nid and nnm:
                            ...
                            # 시트의 헤더(ID, Password, Name...)와 키 값을 완벽히 일치시킴
                            new_row = pd.DataFrame([{
                                "ID": nid, 
                                "Password": hashed_pw, 
                                "Name": nnm, 
                                "BizNo": nid, # 사업자번호는 ID와 동일하게 저장
                                "Email": nem if nem else "", # 빈값이어도 오류 없음
                                "Level": "고객사", 
                ...
                with st.expander("📂 CSV 파일 일괄 등록"):
                    st.info("CSV 필수 헤더: ID, Password, Name (비밀번호는 암호화 전 원문 작성)")
