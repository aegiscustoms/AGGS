...
# ★변경내역★시작★ [시스템 로딩 메시지까지 완벽 차단]
if 'logged_in' not in st.session_state:
    st.session_state.logged_in = False

# 로그인이 안 되어 있다면 시스템 메시지 포함 전체를 투명하게 만듦
if not st.session_state.logged_in:
    st.markdown("""
        <style>
            ...
            /* 2. [핵심] 시스템 로딩 메시지(GSheets... 등)를 투명하게 처리 */
            [data-testid="stStatusWidget"] { visibility: hidden !important; }
            .stStatusWidget { display: none !important; }
            
            /* 3. 전체 배경을 하얀색으로 고정하여 번쩍임 방지 */
            .stApp { background-color: white !important; visibility: hidden; }
            
            /* 4. 로그인 창이 들어가는 구역만 나중에 보이게 설정 */
            div[data-testid="stVerticalBlock"] { visibility: visible; }
        </style>
...
if not st.session_state.logged_in:
    ...
    with login_screen.container():
        # 여기서 visibility를 다시 살려 로그인 창만 보이게 함
        st.markdown("<style>.stApp { visibility: visible !important; }</style>", unsafe_allow_html=True)
