        ...
        with c_p1:
            ...
            with st.form("pw_change_form"):
                ...
                if st.form_submit_button("변경 사항 저장", use_container_width=True):
                    # 1. 컬럼명 대소문자 대응을 위한 전처리
                    df_users_mod = df_users.copy()
                    df_users_mod.columns = [str(c).strip().upper() for c in df_users_mod.columns]
                    ...
                    df_users_mod['ID_CLEAN'] = df_users_mod['ID'].apply(clean_id_val)
                    user_row = df_users_mod[df_users_mod['ID_CLEAN'] == st.session_state.user_id]

                    if not user_row.empty:
                        curr_hash = hashlib.sha256(curr_pw.encode()).hexdigest().lower()
                        # PW 컬럼 혹은 pw 컬럼에서 현재 비밀번호 추출
                        db_pw = str(user_row.iloc[0].get('PASSWORD', user_row.iloc[0].get('PW', user_row.iloc[0].get('pw', '')))).strip().lower()
                        ...
                        if curr_hash == db_pw:
                            if new_pw == new_pw_chk:
                                if len(new_pw) < 4:
                                    ...
                                    # 3. 원본 데이터프레임(df_users) 업데이트 (컬럼명 유연성 확보)
                                    target_id_col = 'ID' if 'ID' in df_users.columns else 'id'
                                    target_pw_col = 'PASSWORD' if 'PASSWORD' in df_users.columns else ('PW' if 'PW' in df_users.columns else 'pw')
                                    
                                    # 실제 행 위치 찾아서 업데이트
                                    mask = df_users[target_id_col].apply(clean_id_val) == st.session_state.user_id
                                    df_users.loc[mask, target_pw_col] = new_hash
                        ...
                        st.error("❌ 유저 정보를 찾을 수 없습니다. 다시 로그인 해주세요.")
