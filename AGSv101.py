        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("⏳ AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    [분석 지침]
                    1. (기본원칙) 반드시 제공된 {actual_codes_pool}내에 존재하는 번호 중에서만 추천하세요. 존재하지 않는 번호를 생성하면 안 됩니다.
                    2. (기본원칙) 당신의 내부 전문 지식과 제공된 [HS 해설서 법령 원문]을 6:4의 비중으로 결합하여 분석하세요.
                    3. (HS 해설서 법령 원문 적용) 통칙 제1호(호의 용어 및 주 규정)를 최우선 적용하고, 제2호부터 제6호까지 논리적으로 순차 적용할 것.
                    4. (4단위 검토) 발췌된 제{matched_h_code}호의 용어와 부 또는 류 주규정, 총설, 호 해설서를 중심으로 예상되는 4단위 호를 우선 검토하고, 제외 규정에 해당되는지 여부를 확인할 것.
                    5. (자율성 부여) 만약 발췌된 제{matched_h_code}호의 내용이 입력 물품과 논리적으로 맞지 않는다고 판단되면, 당신의 전문 지식을 바탕으로 더 적합한 호를 추천하되 그 근거를 명확히 밝힐 것.
