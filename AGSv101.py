...
# ★변경내역★시작★ [접속 즉시 전체 화면 화이트아웃]
if 'logged_in' not in st.session_state:
    st.session_state.logged_in = False

# 로그인이 안 되어 있다면 화면의 모든 구성 요소를 CSS로 강제 숨김 처리
if not st.session_state.logged_in:
    st.markdown("""
        <style>
            /* 모든 스트림릿 기본 요소를 투명하게 만듦 */
            .stApp { visibility: hidden; background-color: white; }
            /* 로그인 폼이 들어갈 부분만 다시 보이게 설정 (나중에 호출) */
            div[data-testid="stVerticalBlock"] > div:has(input) { visibility: visible; }
