...
# 3. [최종 교정] 폰트 및 텍스트 최적화 (13px, 이모지 제거, Logout 표기)
if st.session_state.logged_in:
    st.markdown(f"""
        <style>
            /* 1. 상단 여백 극한으로 제거 */
            .block-container {{ padding-top: 0.5rem !important; padding-bottom: 0rem !important; }}
            .stTabs {{ margin-top: -48px; }} 
            ...
            .user-box {{
                ...
                color: #1E3A8A;
                font-weight: 600;
                font-size: 13px; /* 13px로 조정 */
                white-space: nowrap;
            }}
            
            /* 3. Logout 버튼 스타일 (크기 축소) */
            div[data-testid="stButton"] > button[key="top_logout_btn"] {{
                background-color: #1E3A8A !important;
                color: white !important;
                ...
                font-size: 12px !important; /* 텍스트가 Logout이므로 12px가 적당합니다 */
                width: auto !important;
                border: none !important;
            }}
        </style>
        <div class="custom-top-bar">
            <div class="user-box">{st.session_state.user_name}님</div>
            <div style="width: 75px;"></div> </div>
    """, unsafe_allow_html=True)

    # 실제 Logout 버튼 배치 (비율 미세 조정)
    _, col_logout = st.columns([9.25, 0.75])
    with col_logout:
        st.write("<div style='margin-top: -3px;'>", unsafe_allow_html=True)
        if st.button("Logout", key="top_logout_btn"): # "Logout"으로 변경
            st.session_state.logged_in = False
            try:
