...
# ==========================================
# [최종 통합] 탭 메뉴 및 우측 상단 유저정보 (위치 고정판)
# ==========================================
...
if st.session_state.get('logged_in', False):
    st.markdown(f"""
        <style>
            ...
            /* 탭 메뉴 위치 조정 */
            .stTabs {{ margin-top: -30px; position: relative; z-index: 10; }}

            /* 유저 정보 및 버튼 통합 컨테이너 (우측 상단 절대 위치 고정) */
            .fixed-top-right-bar {{
                position: absolute;
                top: -42px; 
                right: 0px;
                display: flex;
                align-items: center;
                gap: 8px; /* 유저명과 버튼 사이 간격 */
                z-index: 1001;
            }}

            /* 유저명 박스 스타일 (13px, 이모지 제거) */
            .user-name-tag-v2 {{
                background-color: #f8f9fa;
                padding: 4px 10px;
                border-radius: 10px;
        ...
        <div style="position: relative; width: 100%;">
            <div class="fixed-top-right-bar">
                <div class="user-name-tag-v2">{st.session_state.user_name}님</div>
                <div style="width: 75px; height: 28px;"></div>
            </div>
        </div>
    """, unsafe_allow_html=True)

    # 실제 Logout 버튼 (HTML로 확보한 빈 공간에 절대 위치로 강제 배치)
    st.write(f"""
        <div style="position: absolute; top: -43px; right: 0px; z-index: 1002; width: 75px;">
    """, unsafe_allow_html=True)
