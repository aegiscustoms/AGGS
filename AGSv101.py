...
# ★변경내역★시작★ [로그인 전용 가림막 및 논리적 데이터 로드 통합본]
    ...
    with login_screen.container():
        ...
        with cl2:
            with st.form("login_form"):
                ...
                if submit:
                    # [논리적 차단] 버튼 클릭 시점에만 스피너 안에서 데이터를 로드함
                    with st.spinner("보안 인증 및 데이터 연결 중..."):
                        try:
                            # 상단에 있던 로드 로직을 이 위치로 이동
                        ...
                        # 2) 일반 고객사 체크
                            ...
                            if 'ID' in df_check.columns:
                                # ID 클리닝 함수 (8764101210.0 등의 오류 방지)
                                def clean_id(x):
                            ...
                            if df_users is not None:
                                st.error("⚠️ 사용자 데이터가 비어 있습니다.")
    
    # [방어막] 로그인이 안 된 상태라면 여기서 프로그램을 물리적으로 중단
    st.stop()
# ★변경내역★끝★
