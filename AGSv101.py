        ...
        with st.form("login_form"):
            ...
            if submit:
                ...
                # 2) 일반 고객사 체크
                    ...
                    # 입력 비밀번호 암호화
                    hashed_input = hashlib.sha256(input_pw.encode()).hexdigest().lower()
                    
                    # 데이터 전처리 (문자열 변환 및 공백 제거)
                    df_check = df_users.copy()
                    df_check.columns = [c.strip() for c in df_check.columns] # 컬럼명 공백 제거
                    
                    # ID 매칭 확인
                    user_match = df_check[df_check['ID'].astype(str).str.strip() == input_id]
                    
                    if not user_match.empty:
                        db_pw = str(user_match.iloc[0]['Password']).strip().lower()
                        
                        # [진단용 출력] 로그인 실패 시에만 아주 잠깐 정보를 보여줌
                        if hashed_input == db_pw:
                            ...
                            # 관세사님만 확인 가능한 디버깅 정보 (나중에 삭제 가능)
                            with st.expander("🔍 진단 정보 (비밀번호 불일치 원인 확인)"):
                                st.write(f"입력값 해시: {hashed_input[:10]}...")
                                st.write(f"DB 저장 해시: {db_pw[:10]}...")
                                st.info("두 값이 다르면 '초기화' 버튼을 다시 눌러주세요.")
                    else:
