...
# --- [125행 ~ 165행 부근: 로그인 로직 최종 보수적 버전] ---
try:
    conn_gs = st.connection("gsheets", type=GSheetsConnection)
    # 캐시를 완전히 무시하고 시트 전체를 읽어옴
        ...
        with st.form("login_form"):
            ...
            if submit:
                ...
                # 2) 일반 고객사 체크
                    ...
                    # 시트 데이터 전처리: 모든 컬럼명을 대문자로 통일하고 공백 제거
                    df_check = df_users.copy()
                    df_check.columns = [str(c).strip().upper() for c in df_check.columns]
                    
                    # 'ID' 컬럼이 존재하는지 확인 (대문자로 변경했으므로 'ID'로 체크)
                    if 'ID' in df_check.columns:
                        # 모든 데이터를 문자열로 변환 후 비교
                        df_check['ID'] = df_check['ID'].astype(str).str.strip().str.replace("-", "")
                        
                        # 아이디가 일치하는 행 찾기
                        user_match = df_check[df_check['ID'] == input_id]
                        
                        if not user_match.empty:
                            # 비밀번호 비교 (PASSWORD 컬럼도 대문자로 체크)
                        ...
                        st.error("⚠️ 시트에서 'ID' 컬럼을 찾을 수 없습니다. 헤더를 확인해주세요.")
                else:
                    st.error("⚠️ 사용자 데이터베이스가 비어있습니다.")
    st.stop()
# ◆변경내역◆끝◆
