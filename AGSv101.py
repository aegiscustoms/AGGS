...
# --- [125행 ~ 160행 부근: 로그인 로직 최종 무결점 버전] ---
try:
    conn_gs = st.connection("gsheets", type=GSheetsConnection)
    # ttl=0을 통해 매번 구글 시트에서 최신 데이터를 강제로 새로 읽어옵니다.
                ...
                # 2) 일반 고객사 체크
                    ...
                    if not df_users.empty:
                        ...
                        if not user_match.empty:
                            ...
                            # 디버깅 도움말: 비밀번호 초기화 후에도 안 된다면 시트의 헤더를 다시 확인해야 함
                            st.error("❌ 아이디 또는 비밀번호가 일치하지 않습니다.")
                    else:
                        st.error("⚠️ 사용자 정보를 불러올 수 없습니다. 시트를 확인해주세요.")
    st.stop()
# ◆변경내역◆끝◆
