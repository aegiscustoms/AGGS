...
with tabs[1]:
    ...
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            ...
            # 1. 계층별 품명 정보 (4, 6, 7, 10단위 추출 로직)
            # 입력된 10자리를 잘라 상위 부호들을 리스트화
            hsk_prefixes = [hsk[:4], hsk[:6], hsk[:7], hsk]
            hsk_prefixes = list(dict.fromkeys(hsk_prefixes)) # 중복 제거
            
            # SQL 파라미터를 사용하여 계층별 마스터 정보 조회
            m_query = f"SELECT hs_code, name_kr, name_en FROM hs_master WHERE hs_code IN ({','.join(['?']*len(hsk_prefixes))}) ORDER BY length(hs_code) ASC"
            m = pd.read_sql(m_query, conn, params=hsk_prefixes)
            
            # 2. 기타 상세 정보 (표준품명, 세율, 세관장확인)
            ...
            # 10단위 마스터 정보가 존재하는지 확인
            if not m[m['hs_code'] == hsk].empty:
                st.subheader(f"📋 HS {hsk} 상세 정보 리포트")
                
                # [품명 섹션]
                c1, c2 = st.columns(2)
                with c1:
                    st.info("**HS 계층별 국문 품명**")
                    # 4, 6, 7, 10단위 순차 출력
                    for _, row in m.iterrows():
                        label = f"{len(row['hs_code'])}단위"
                        st.write(f"- **{label} ({row['hs_code']}):** {row['name_kr']}")
                    
                    # 10단위 영문명 별도 강조
                    m10 = m[m['hs_code'] == hsk]
                    if not m10.empty:
                        st.write(f"- **영문(10단위):** {m10['name_en'].values[0]}")
                ...
                # [세율 섹션]
                st.divider()
                st.markdown("### 💰 관세율 분류 정보")
                if not r.empty:
                    # 분류: A(기본), C(WTO), F(협정), 기타
                    ra = r[r['type'] == 'A']
                    rc = r[r['type'] == 'C']
                    ...
                    m1, m2 = st.columns(2)
                    # metric 출력 시 f-string을 사용하여 타입 오류(numpy) 방지
                    m1.metric("기본세율 (A)", f"{ra['rate'].values[0]}%" if not ra.empty else "-")
                    m2.metric("WTO협정세율 (C)", f"{rc['rate'].values[0]}%" if not rc.empty else "-")
                    
                    st.write("**협정세율 (F)**")
                    st.dataframe(rf, hide_index=True, use_container_width=True)
                    
                    st.write("**기타세율 (간이/내국세 등)**")
                ...
                st.error("입력하신 10단위 HS코드의 마스터 정보가 DB에 없습니다. [관리자] 탭에서 'HS코드(마스터)' 파일을 업로드해 주세요.")
        else:
            st.warning("조회할 HS코드를 입력해 주세요.")
...
if st.session_state.is_admin:
    with tabs[-1]:
        ...
        **탭 2의 계층별 품명 출력**을 위해 'HS코드(마스터)' 파일을 가장 먼저 업로드하는 것을 권장합니다.
        """)
        ...
        if up_file and st.button(f"{mode} DB 반영 실행", use_container_width=True):
            try:
                ...
                st.warning("CSV 파일의 헤더명이나 형식을 확인해 주세요.")
