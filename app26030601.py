...
HTTP_HEADERS = {"User-Agent": "Mozilla/5.0", "Accept": "application/xml"}
...
# --- [Tab 2] HS정보 (가이드 API018 준수) ---
with tabs[1]:
    st.markdown(f"<div style='font-size: {TITLE_FONT_SIZE}; font-weight: bold; color: #1E3A8A;'>📘 실시간 HS부호 및 세율 (API018)</div>", unsafe_allow_html=True)
            ...
            with st.spinner("관세청 데이터 호출 중..."):
                ...
                try:
                    ...
                    if root.find(".//korePrnm") is not None:
                        st.info(f"✅ 한글품명: {root.findtext('.//korePrnm')}")
                        st.write(f"영문품명: {root.findtext('.//englPrnm')}")
                        st.success(f"적용세율: {root.findtext('.//txrt') or '정보없음'}")
                        st.write(f"수량/중량단위: {root.findtext('.//qtyUt')} / {root.findtext('.//wghtUt')}")
                    else: st.warning("정보를 찾을 수 없습니다. 인증키 승인 여부를 확인하세요.")
                except Exception as e: st.error(f"연결 오류: {e}")

# --- [Tab 3] 통계부호 (가이드 API019 준수) ---
with tabs[2]:
