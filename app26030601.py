...
with tabs[0]:
    ...
    if st.button("HS분석 실행", use_container_width=True):
        with st.spinner("AI 분석 중..."):
            try:
                prompt = "당신은 전문 관세사입니다. 입력된 정보에 대한 HS코드 10자리를 추천하세요."
                res = model.generate_content([prompt, Image.open(u_img)] if u_img else [prompt, u_input])
                st.write(res.text)
            except Exception as e: st.error(f"오류: {e}")

# --- [Tab 2] HS정보 (API018 가이드 정밀 반영) ---
with tabs[1]:
    st.markdown(f"<div style='font-size: {TITLE_FONT_SIZE}; font-weight: bold; color: #1E3A8A;'>📘 실시간 HS부호 정보 (API018)</div>", unsafe_allow_html=True)
    RATE_KEY = st.secrets.get("RATE_API_KEY", "").strip()
    target_hs = st.text_input("HS코드 10자리", placeholder="예: 0302440000", key="hs_rate_api")
    if st.button("실시간 HS 정보 조회", use_container_width=True):
        if target_hs:
            with st.spinner("서버 응답 확인 중..."):
                ...
                try:
                    ...
                    # 서버 에러 메시지 확인
                    errMsg = root.findtext(".//ntceCn")
                    if errMsg: st.error(f"서버 메시지: {errMsg}")
                    
                    k_name = root.findtext(".//korePrnm")
                    if k_name:
                        st.info(f"✅ 품명: {k_name}")
                        st.write(f"영문: {root.findtext('.//englPrnm')}")
                        st.success(f"세율: {root.findtext('.//txrt')}")
                    elif not errMsg: st.warning("정보를 찾을 수 없습니다. 인증키 권한을 확인하세요.")
                except Exception as e: st.error(f"연결 오류: {e}")

# --- [Tab 3] 통계부호 (API019 가이드 정밀 반영) ---
with tabs[2]:
    st.markdown(f"<div style='font-size: {TITLE_FONT_SIZE}; font-weight: bold; color: #1E3A8A;'>📊 실시간 통계부호 검색 (API019)</div>", unsafe_allow_html=True)
    ...
    with col2: kw = st.text_input("검색 키워드", placeholder="예: 정밀전자")
    if st.button("부호 실시간 검색", use_container_width=True):
        ...
        try:
            ...
            if codes:
                res_list = [{"부호": c.findtext("statsSgn"), "내역": c.findtext("koreBrkd")} for c in codes if not kw or kw in c.findtext("koreBrkd")]
                st.dataframe(pd.DataFrame(res_list), use_container_width=True)
            elif not errMsg: st.warning("결과가 없습니다.")
        except Exception as e: st.error(f"연결 오류: {e}")

# --- [Tab 4] 화물통관진행정보 (고객님 고정 코드 및 보안 강화) ---
with tabs[3]:
    st.subheader("📦 실시간 화물통관 진행정보 조회")
    CR_API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
    bl_no = st.text_input("B/L 번호 입력", key="bl_track_v4")
    if st.button("실시간 추적", use_container_width=True) and bl_no:
        url = "https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry/retrieveCargCsclPrgsInfo"
        params = {"crkyCn": CR_API_KEY, "blYy": "2026", "hblNo": bl_no.strip().upper()}
        try:
            ...
            elif not errMsg: st.warning("정보 없음")
