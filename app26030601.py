...
# --- [Tab 5] 세액계산기 (관세사 실무 로직 반영) ---
with tabs[4]:
    st.markdown(f"<div style='font-size: {TITLE_FONT_SIZE}; font-weight: bold; color: #1E3A8A;'>🧮 수입물품 예상 세액계산기</div>", unsafe_allow_html=True)
    
    # 1. 과세표준(CIF) 설정 섹션
    with st.expander("📍 1. 과세가격(CIF) 입력", expanded=True):
        col1, col2 = st.columns(2)
        with col1:
            item_price = st.number_input("물품가격(외화)", min_value=0.0, step=100.0, help="FOB 가격 등을 입력하세요.")
            exchange_rate = st.number_input("적용 환율", min_value=1.0, value=1350.0, help="세관 고시환율을 입력하세요.")
        with col2:
            freight = st.number_input("운임(Freight)", min_value=0, value=0)
            insurance = st.number_input("보험료(Insurance)", min_value=0, value=0)
    ...
    # 2. 세율 설정 섹션
    with st.expander("📍 2. 세종별 세율 설정", expanded=True):
        c1, c2, c3 = st.columns(3)
        with c1:
            duty_rate = st.number_input("관세율 (%)", min_value=0.0, value=8.0)
            duty_reduction = st.number_input("관세감면율 (%)", min_value=0.0, max_value=100.0, value=0.0)
        with c2:
            excise_rate = st.number_input("개소세/주세 (%)", min_value=0.0, value=0.0)
            edu_rate = st.number_input("교육세율 (%)", min_value=0.0, value=30.0 if excise_rate > 0 else 0.0)
        with c3:
            v_rate = st.number_input("부가세율 (%)", min_value=0.0, value=10.0)
            st.caption("※ 농특세는 관세/개소세 감면 시 자동 계산 로직에 포함됩니다.")

    # 3. 계산 실행 및 결과 출력
    if st.button("세액 계산 실행", use_container_width=True):
        # [Step 1] 관세 계산
        duty_raw = int(cif_krw * (duty_rate / 100))
        duty_final = int(duty_raw * (1 - (duty_reduction / 100))) # 감면 적용 후 실납부액
        
        # [Step 2] 개별소비세/주세 (과표: CIF + 관세액)
        # ※ 주의: 감면 전 관세액을 더하는 것이 일반적이나 실무에 따라 다를 수 있음
        excise_base = cif_krw + duty_raw 
        excise_tax = int(excise_base * (excise_rate / 100))
        
        # [Step 3] 교육세 (과표: 개소세/주세 납부액)
        edu_tax = int(excise_tax * (edu_rate / 100))
        
        # [Step 4] 농어촌특별세 (관세 감면액의 20% + 개소세 감면분 등)
        # 관세감면분에 대한 농특세만 우선 반영 (감면액의 20%)
        rural_tax = int((duty_raw - duty_final) * 0.2)
        
        # [Step 5] 부가가치세 (과표: CIF + 관세 + 개소세 + 교육세 + 농특세)
        vat_base = cif_krw + duty_final + excise_tax + edu_tax + rural_tax
        vat_tax = int(vat_base * (v_rate / 100))
        
        # 합계
        total_tax = duty_final + excise_tax + edu_tax + rural_tax + vat_tax

        # 결과 리포트 출력
        
        st.success("### 📊 산출 결과 리포트")
        
        res_data = {
            "항목": ["1. 관세 (Duty)", "2. 개별소비세/주세", "3. 교육세", "4. 농어촌특별세", "5. 부가가치세 (VAT)"],
            "과세표준(원)": [f"{cif_krw:,.0f}", f"{excise_base:,.0f}", f"{excise_tax:,.0f}", f"{(duty_raw-duty_final):,.0f}", f"{vat_base:,.0f}"],
            "납부세액(원)": [duty_final, excise_tax, edu_tax, rural_tax, vat_tax]
        }
        
        df_res = pd.DataFrame(res_data)
        st.table(df_res.style.format({"납부세액(원)": "{:,.0f}"}))
        
        st.metric(label="💰 총 납부 세액 합계", value=f"{total_tax:,.0f} 원")
        st.info("※ 본 계산 결과는 참고용이며, 실제 신고 시 과세표준 및 가산세 여부에 따라 달라질 수 있습니다.")
