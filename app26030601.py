...
# --- [Tab 2] HS정보 (가이드 API018 최신 API 버전) ---
with tabs[1]:
...
# --- [Tab 3] 통계부호 (가이드 API019 최신 API 버전) ---
with tabs[2]:
...
# --- [Tab 6] 관리자 페이지 (사용자 계정 관리 전용) ---
if st.session_state.is_admin:
    with tabs[-1]:
        st.header("⚙️ 사용자 계정 승인 관리")
        conn = sqlite3.connect("users.db")
