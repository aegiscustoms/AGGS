...
def load_hs_resources():
    ...
    return df_hs, df_sub_hs, manual_text # df_sub_hs
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    - "4. 추천 HS Code내 분류가능한 HSK" 항목 작성 시:
                      [참조용 HSK 리스트 데이터]를 확인하여, 분석된 6단위 소호와 논리적으로 일치하는 10단위 HSK들을 모두 나열하세요. 
                      만약 데이터가 부족하면 추천 6단위에 '-0000'을 붙여 가상의 HSK로 표기하지 말고, 데이터에 있는 실제 번호를 우선적으로 제시하세요.
