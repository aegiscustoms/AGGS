...
with tabs[1]:
    ...
    if target_hs != "0000000000":
        ...
        with col_cp1:
            st.write("📥 **수입요령**")
            # 앞자리 일치 검색 로직
            cp_im_res = df_c_im[df_c_im.apply(lambda x: target_hs.startswith(x['HS_code']), axis=1)][['Law_name', 'Name', 'Guide']]
            cp_im_res.columns = ['법령명', '대상', '요령']
            st.dataframe(cp_im_res.sort_values('법령명'), use_container_width=True, hide_index=True)

        with col_cp2:
            st.write("📤 **수출요령**")
            cp_ex_res = df_c_ex[df_c_ex.apply(lambda x: target_hs.startswith(x['HS_code']), axis=1)][['Law_name', 'Name', 'Guide']]
            cp_ex_res.columns = ['법령명', '대상', '요령']
            st.dataframe(cp_ex_res.sort_values('법령명'), use_container_width=True, hide_index=True)
