            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    - "4. 추천 HS Code내 분류가능한 HSK" 항목 작성 지침:
                        1) 먼저 "2. 추천 HS Code"에서 확정한 6단위 번호를 가져오세요. (예: 8470.10)
                        2) **중요: 가져온 6단위 번호에서 마침표(.)를 제거하여 순수 숫자 6자리만 만드세요.** (예: 8470.10 -> 847010)
                        3) 위에서 만든 **숫자 6자리**와 [참조용 HSK 리스트 데이터]의 CODE 앞 6자리를 비교하여 일치하는 행을 모두 찾으세요.
                        4) 추출된 데이터는 다시 관세사님이 원하시는 **'XXXX.XX-XXXX 국문품명 [영문품명]'** 형식으로 예쁘게 가공하여 출력하세요.
                        5) 데이터가 있다면 리스트를 나열하고, 없다면 "10단위 상세 HSK는 전문관세사에게 문의바랍니다."라고 작성하세요.                        
