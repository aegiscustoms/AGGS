...
with tabs[2]:
    ...
    # 1) 자동완성 셀렉트박스 (STAT_LIST는 기존 정의된 리스트 사용)
    ...
    # 2) 키워드 검색 (한글내역 내 키워드 검색)
    search_kw = st.text_input(
        "키워드 검색", 
        placeholder="검색할 한글 명칭(내역)을 입력하세요",
        help="입력하신 키워드가 포함된 모든 데이터를 검색하여 코드를 보여줍니다."
    )
    
    if st.button("부호 조회", use_container_width=True):
        if search_kw:
            # 검색어 내 공백을 %로 변환하여 유연하게 검색 (예: '부칙 7조' -> '부칙%7조')
            ...
            if not res.empty:
                ...
                st.warning(f"'{target_cat}' 카테고리에 '{search_kw}'를 포함하는 명칭이 없습니다. 관리자 페이지에서 데이터를 다시 업로드해 주세요.")
        else:
...
if st.session_state.is_admin:
    with tabs[-1]:
        ...
        # 구역 2: 통계부호 (일괄 업로드)
        st.subheader("📊 2. 통계부호 관리 (드래그 일괄 업로드)")
        st.info("💡 관세청 CSV 파일 구조(No, 코드, 명칭)에 맞춰 자동으로 2번째와 3번째 열을 추출합니다.")
        
        multi_files = st.file_uploader(
            "통계부호 CSV 파일들을 드래그하세요 (파일명이 카테고리명과 일치해야 함)", 
            type="csv", 
            accept_multiple_files=True
        )
        
        if multi_files and st.button("🚀 통계부호 일괄 업로드 실행", use_container_width=True):
            conn = sqlite3.connect("customs_master.db")
            total = 0
            success_list = []
            
            for f in multi_files:
                cat = f.name.split('.')[0] # 파일명에서 카테고리 추출
                if cat in STAT_LIST:
                    try:
                        # 다양한 CSV 인코딩 대응
                        try:
                        ...
                        # [핵심 수정] 관세청 파일 구조 대응: 
                        # 보통 0번:No, 1번:상세통계부호(코드), 2번:한글내역(명칭)
                        # 따라서 iloc[:, [1, 2]]를 사용하여 데이터 추출
                        if len(df.columns) >= 3:
                            df_stat = df.iloc[:, [1, 2]].copy()
                        else:
                            # 열이 2개뿐인 경우 (코드, 명칭만 있는 경우)
                            df_stat = df.iloc[:, [0, 1]].copy()
                            
                        df_stat.columns = ['code', 'name']
                        df_stat['category'] = cat
                        
                        # 기존 해당 카테고리 데이터 삭제 후 새로 삽입
            ...
            conn.close()
            if success_list:
                st.success(f"총 {total}개의 통계부호 파일 반영 완료: {', '.join(success_list)}")
            else:
                st.error("업로드된 파일 중 유효한 통계부호 카테고리명이 없습니다. 파일명을 확인해 주세요.")
