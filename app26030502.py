...
# --- [Tab 1] HS검색 (출력 형식 1. 2. 변경 및 줄바꿈 강화) ---
with tabs[0]:
    col_a, col_b = st.columns([2, 1])
    with col_a: 
        # 입력창 명칭 고정
        u_input = st.text_input("품명/물품정보(용도/기능/성분/재질) 입력", key="hs_q")
    with col_b: 
        u_img = st.file_uploader("이미지 업로드", type=["jpg", "png", "jpeg"], key="hs_i")
    ...
    if st.button("HS분석 실행", use_container_width=True):
        if u_img or u_input:
            with st.spinner("전문 관세사 AI가 분석 중..."):
                try:
                    # 넘버링 및 줄바꿈 지침 강화 프롬프트
                    prompt = f"""당신은 전문 관세사입니다. 아래 지침에 따라 HS코드를 분류하고 리포트를 작성하세요.
                    
                    [분석 지침]
                    - 사용자 입력값('{u_input}')의 용도, 기능, 성분, 재질 정보를 정밀 검토하세요.
                    - 이미지와 텍스트를 종합하여 가장 적합한 10자리 HSK 코드를 도출하세요.

                    [출력 형식 및 순서 지침]
                    반드시 아래 형식을 유지하고, 항목 간에는 줄바꿈 적용해주세요.
                    ...
                    2. 추천결과:
                       ...
                       - 미확정인 경우: 상위 3순위까지 추천하되, 3순위 확률이 낮으면 2순위까지만 출력.
                       - 각 순위는 반드시 줄바꿈하여 한 줄에 하나씩 표기하세요.
                       - 형식: "n순위 [코드] [확률]%"
                    ...
                    if u_input: content.append(f"상세 물품정보: {u_input}")
                    ...
                    # 100% 확정 시 하단에 DB 상세 정보 연동 (고정 로직)
...
# --- [Tab 3] 통계부호 (자동# --- [Tab 3] 통계부호 (검색 범위 확장 및 출력 최적화) ---
with tabs[2]:
    st.subheader("📊 통계부호 통합 검색")
    
    # 1) 카테고리 선택 (placeholder 개념의 도움말 추가)
    target_cat = st.selectbox(
        "검색할 통계부호 카테고리를 입력/선택하세요", 
        STAT_LIST, 
        index=0,
        help="예: 보세구역부호 등 (원하는 부호 명칭을 타이핑하면 자동완성됩니다.)"
    )
    
    # 2) 키워드 검색창 (placeholder 추가)
    search_kw = st.text_input(
        "키워드 검색", 
        placeholder="검색할 키워드를 입력하세요 (예: 정밀전자산업, 자동자료처리기기 등)",
        help="관세감면코드의 경우 한글내역과 대상물품 모두를 검색합니다."
    )
    ...
    if st.button("부호 조회", use_container_width=True):
        if search_kw:
            ...
            # 관세감면분납코드 등 3개 열(코드, 내역, 물품)이 있는 경우를 대비한 유연한 쿼리
            # category가 '관세감면분납코드'인 경우 name(한글내역)과 item(대상물품) 컬럼 모두 검색
            if target_cat == "관세감면분납코드":
                query = """
                    SELECT code as '통계코드', name as '한글내역', item as '대상물품' 
                    FROM stat_codes 
                    WHERE category = ? AND (name LIKE ? OR item LIKE ?)
                """
            ...
            if not res.empty:
                ...
                st.warning(f"'{target_cat}' 카테고리에 '{search_kw}'를 포함하는 결과가 없습니다.")
        else:
...
if st.session_state.is_admin:
    with tabs[-1]:
        ...
        # --- 구역 2: 통계부호 (일괄 업로드 및 열 자동 매핑) ---
        st.divider()
        st.subheader("📊 2. 통계부호 전용 관리 (드래그 일괄 업로드)")
        st.info("""
        💡 **업로드 가이드:**
        1. 여러 개의 통계부호 CSV 파일을 한 번에 드래그하여 아래 박스에 넣으세요.
        2. **파일명은 반드시 통계부호 명칭(예: 관세감면분납코드.csv)과 일치**해야 합니다.
        3. 관세청 표준 양식(No, 코드, 명칭...)을 자동으로 감지하여 '대상물품' 열까지 저장합니다.
        """)
        
        # 다중 파일 업로드 (드래그 앤 드롭)
        multi_files = st.file_uploader(
            "통계부호 CSV 파일들을 드래그하세요", 
            type="csv", 
            accept_multiple_files=True,
            key="stat_bulk_uploader"
        )
        
        if multi_files:
            if st.button("🚀 통계부호 일괄 업로드 실행", use_container_width=True):
                conn = sqlite3.connect("customs_master.db")
                total_files = 0
                success_list = []
                ...
                for f in multi_files:
                    ...
                    if cat in STAT_LIST:
                        try:
                            ...
                            # --- 열(Column) 매핑 최적화 로직 ---
                            if cat == "관세감면분납코드":
                                # 관세감면분납코드: No(0), 상세통계부호(1), 한글내역(2), ..., 대상물품(6)
                                if len(df.columns) >= 7:
                                    # 2번째(index 1), 3번째(index 2), 7번째(index 6) 열 추출
                                    df_stat = df.iloc[:, [1, 2, 6]].copy()
                                    df_stat.columns = ['code', 'name', 'item']
                                else:
                                    # 열이 부족할 경우 대비 안전 장치
                                ...
                                if len(df.columns) >= 3:
                                    ...
                                    df_stat.columns = ['code', 'name']
                                df_stat['item'] = None # 대상물품 컬럼 비움
                            
                            # 카테고리 정보 추가
                            df_stat['category'] = cat
                            
                            # 해당 카테고리의 기존 데이터 삭제 후 삽입 (덮어쓰기 효과)
                            ...
                            st.error(f"'{cat}' 파일 처리 중 오류 발생: {e}")
                    else:
                        st.warning(f"⚠️ '{f.name}' 파일명은 유효한 통계부호 카테고리에 없습니다. (목록 확인 필요)")
                ...
                if success_list:
                    st.balloons()
                    st.success(f"✅ 총 {total_files}개의 통계부호가 성공적으로 반영되었습니다.")
                    with st.expander("업로드 성공 목록 보기"):
                        st.write(", ".join(success_list))
