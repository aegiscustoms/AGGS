...
# --- [Tab 3] 통계부호 (오류 수정 및 폰트 사양 반영) ---
with tabs[2]:
    st.markdown("""
        <style>
            .tab3-title { font-size: 15px !important; font-weight: bold; color: #1E3A8A; margin-bottom: 5px; }
            .tab3-text { font-size: 12px !important; }
            div[data-testid="stDataFrame"] td, div[data-testid="stDataFrame"] th { font-size: 12px !important; }
        </style>
    """, unsafe_allow_html=True)
    ...
    if st.button("부호 조회", use_container_width=True):
        if search_kw:
            ...
            if target_cat == "관세감면분납코드":
                query = "SELECT code as '상세통계부호', name as '한글내역', item as '대상물품' FROM stat_codes WHERE category = ? AND (name LIKE ? OR item LIKE ?)"
                res = pd.read_sql(query, conn, params=(target_cat, clean_kw, clean_kw))
            elif target_cat == "내국세면세부호":
                query = "SELECT sub_cat as '구분명', code as '내국세부가세감면코드', name as '내국세부가세감면명', item as '대상물품' FROM stat_codes WHERE category = ? AND (name LIKE ? OR item LIKE ?)"
                res = pd.read_sql(query, conn, params=(target_cat, clean_kw, clean_kw))
            else:
...
if st.session_state.is_admin:
    with tabs[-1]:
        ...
        st.divider()
        st.subheader("📊 2. 통계부호 일괄 관리 (열 매핑 보완)")
        multi_files = st.file_uploader("CSV 파일 드래그", type="csv", accept_multiple_files=True)
            ...
            for f in multi_files:
                ...
                if cat in STAT_LIST:
                    try:
                        ...
                        if cat == "내국세면세부호":
                            df_stat = df.iloc[:, [0, 1, 2, 4]].copy()
                            df_stat.columns = ['code', 'name', 'item', 'sub_cat']
                        elif cat == "관세감면분납코드":
                            df_stat = df.iloc[:, [1, 2, 6]].copy()
                            df_stat.columns = ['code', 'name', 'item']
                            df_stat['sub_cat'] = None
                        else:
