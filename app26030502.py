...
def init_db():
    ...
    # 통계부호 테이블 (내부적으로 item은 저장하되 출력에서 제외 가능)
...
with tabs[2]:
    ...
    if st.button("부호 조회", use_container_width=True):
        if search_kw:
            ...
            # [수정] 요청하신 출력 열 순서 및 명칭 반영
            if target_cat == "관세감면분납코드":
                query = "SELECT code as '상세통계부호', name as '한글내역' FROM stat_codes WHERE category = ? AND (name LIKE ? OR item LIKE ?)"
                res = pd.read_sql(query, conn, params=(target_cat, clean_kw, clean_kw))
            elif target_cat == "내국세면세부호":
                # 구분명(sub_cat) 우선 출력
                query = "SELECT code as '상세통계부호', sub_cat as '구분명', name as '한글내역' FROM stat_codes WHERE category = ? AND (name LIKE ? OR item LIKE ?)"
                res = pd.read_sql(query, conn, params=(target_cat, clean_kw, clean_kw))
            else:
