...
# --- [Tab 2] HS정보 (기존 탭2 명칭 변경 & 12px & 0보정) ---
with tabs[1]:
    st.markdown("<style>.stMarkdown { font-size: 12px !important; }</style>", unsafe_allow_html=True)
    target_hs = st.text_input("조회할 HSK 10자리 (숫자만)", key="hs_info_q", placeholder="예: 0101211000")
    if st.button("HS 상세 조회"):
        if target_hs:
            ...
            r = pd.read_sql(f"""SELECT r.type as '세율코드', n.h_name as '세율명칭', r.rate as '세율' 
                               FROM rates r LEFT JOIN rate_names n ON r.type = n.code 
                               WHERE r.hs_code = '{hsk}'""", conn)
            ...
            if not m.empty:
                ...
                with c2:
                    st.write("**[표준 품명]**")
                    if not std.empty:
                        st.write(f"품명(B): {std['base_name'].values[0]}")
                        st.write(f"표준(E): {std['std_name_kr'].values[0]}")
                    else: st.write("정보 없음")
                ...
                if not r.empty:
                    ...
                    st.dataframe(r.style.set_properties(**{'text-align': 'center'}), hide_index=True, use_container_width=True)
...
if st.session_state.is_admin:
    with tabs[-1]:
        ...
        for i, m_name in enumerate(m_list):
            with cols[i%3]:
                st.write(f"**{m_name}**")
                up = st.file_uploader(f"{m_name}", type="csv", key=f"admin_{m_name}", label_visibility="collapsed")
                if up:
                    if st.button(f"반영", key=f"btn_{m_name}"):
                        df = pd.read_csv(up, encoding='utf-8-sig')
                        conn = sqlite3.connect("customs_master.db")
                        # 각 모드별 매핑 로직 (생략 - 이전과 동일하되 zfill(10) 적용)
                        if "HS부호" in df.columns or "품목번호" in df.columns:
                            col_hs = "HS부호" if "HS부호" in df.columns else "품목번호"
                            df[col_hs] = df[col_hs].astype(str).str.replace(r'[^0-9]', '', regex=True).str.zfill(10)
                        # (테이블 저장 로직...)
                        st.success("반영 완료")
        ...
        with c1:
            if multi_files and st.button("🚀 선택한 모든 파일 일괄 업로드 실행", use_container_width=True):
                conn = sqlite3.connect("customs_master.db")
                success_list = []
