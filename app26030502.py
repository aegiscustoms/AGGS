...
with tabs[1]:    
    ...
    st.markdown(f"""
        <style>
            ...
            .custom-header {{ font-size: {font_size_header} !important; font-weight: bold; color: #1E3A8A; margin-bottom: 8px; }}
            .custom-value {{ font-size: {font_size_value} !important; font-weight: 500; background-color: #F8FAFC; padding: 4px; border-radius: 4px; }}
            /* 데이터프레임 헤더 및 셀 텍스트 크기 조절 */
            .stDataFrame div[data-testid="stTable"] {{ font-size: 12px !important; }}
        </style>
    """, unsafe_allow_html=True)

    # 4) HS코드 10자리 입력 안내
    target_hs = st.text_input("조회할 HSK 10자리를 입력하세요 (0 포함)", key="stat_q", placeholder="예: 0101211000")
    
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            # 검색어 보정: 숫자만 추출 후 부족한 앞자리를 0으로 채워 10자리로 만듦
