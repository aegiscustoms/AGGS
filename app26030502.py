...
with tabs[2]:
    ...
    # 1. 카테고리 선택 (selectbox 내 placeholder 성격의 도움말 추가)
    # Streamlit selectbox는 자체적인 placeholder가 없으므로 help와 label을 통해 가이드 제공
    target_cat = st.selectbox(
        "검색할 카테고리 입력/선택", 
        STAT_LIST, 
        index=0,
        help="예: 보세구역부호 (원하는 부호를 타이핑하여 검색하세요)"
    )
    
    # 2. 키워드 검색 (placeholder 적용)
    search_kw = st.text_input(
        "키워드 검색", 
        placeholder="검색할 키워드 입력 (예: 정밀전자산업, 농업용 등)",
        key="stat_search_input"
    )
    ...
    if st.button("부호 조회", use_container_width=True):
        if search_kw:
            ...
            if target_cat == "관세감면분납코드":
                # 요구사항: 상세통계부호, 한글내역, 대상물품
                query = """
                    SELECT code as '상세통계부호', name as '한글내역', item as '대상물품' 
                    FROM stat_codes 
                ...
                # 요구사항: 구분명, 내국세부가세감면코드, 내국세부가세감면명, 대상물품
                query = """
                    SELECT sub_cat as '구분명', code as '내국세부가세감면코드', 
                           name as '내국세부가세감면명', item as '대상물품' 
                    FROM stat_codes 
                    WHERE category = ? AND (name LIKE ? OR item LIKE ?)
                """
                res = pd.read_sql(query, conn, params=(target_cat, clean_kw, clean_kw))
            ...
            if not res.empty:
                ...
                # 데이터프레임 스타일 적용 (가운데 정렬 및 지정 폰트 유지)
                st.dataframe(
                ...
                st.warning(f"'{target_cat}' 카테고리에 '{search_kw}'를 포함하는 검색 결과가 없습니다.")
        else:
