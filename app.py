...
def init_db():
    ...
    # DB 컬럼 누락 방지 (DatabaseError 해결)
    cursor = conn_auth.execute("PRAGMA table_info(users)")
    existing_cols = [info[1] for info in cursor.fetchall()]
    needed_cols = ['phone', 'email', 'biz_name', 'biz_no', 'biz_rep', 'biz_type', 'biz_item', 'biz_addr', 'tax_email']
    for col in needed_cols:
        if col not in existing_cols:
            conn_auth.execute(f"ALTER TABLE users ADD COLUMN {col} TEXT")

    # 관리자 계정 설정 (aegis01210)
    admin_id = "aegis01210"
...
if not st.session_state.logged_in:
    ...
    else: # 회원가입 생략 (기본 폼 유지)
        st.info("회원가입 후 관리자 승인을 기다려주세요.")
    st.stop()
...
# 탭 구성
tabs_list = ["🔍 HS검색", "📊 통계부호", "🌎 세계 HS/세율", "📜 FTA정보", "📦 화물통관진행정보", "🧮 세액계산기"]
if st.session_state.is_admin: tabs_list.append("⚙️ 관리자")
...
if st.session_state.is_admin:
    with tabs[-1]:
        st.header("⚙️ 운영 관리 (aegis01210)")
        conn = sqlite3.connect("users.db")
        p = pd.read_sql("SELECT id, name, biz_name, is_approved FROM users WHERE is_approved=0", conn)
        st.table(p)
