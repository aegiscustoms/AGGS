...
with tabs[0]:
    ...
    with col1: u_input = st.text_input("품명/정보 입력")
    with col2: u_img = st.file_uploader("이미지", type=["jpg", "png"])
    if u_img: st.image(Image.open(u_img), width=300)
    if st.button("분석 실행", use_container_width=True):
        with st.spinner("AI 분석 중..."):
            prompt = f"품명 출력: 입력('{u_input}')이 있으면 출력, 없으면 예상품명 제시. 100% 확신 시 10자리 코드+(100%) 표기. 미확정 시 6단위 기준 3순위까지 확률 표기. 추천결과: [코드] [확률] 필수."
