...
# --- [Tab 2] HS정보 (요청하신 정밀 매핑 및 10cm 고정 디자인 적용) --- #변경내역★★
with tabs[1]:
        ...
        if target_hs:
            ...
            try:
                ...
                if not m.empty:
                    ...
                    ci, ce = st.columns(2)
                    # 요건 테이블 또한 화면폭에 맞춰 출력 (항목명: 법령명, 승인기관, 서류명) #변경내역★★
                    with ci: 
                        st.markdown("[수입 요건]")
                        st.write(req_i.to_html(index=False, classes='full-width-table'), unsafe_allow_html=True)
                    with ce: 
                        st.markdown("[수출 요건]")
                        st.write(req_e.to_html(index=False, classes='full-width-table'), unsafe_allow_html=True)
                else: 
